...
if type(Rayfield) == "table" then
    -- try to find Main tab object; assume variable 'MainTab' or similar exists in scope.
    local MainTab = nil
    -- common variable names used in scripts: Main, MainTab, MainTabSection, Tab, MainTabSection
    for _, v in pairs(_G) do
        -- nothing: don't rely on _G; instead try to reference known 'Main' variable if present
    end
    -- fallback: try to get the tab from Window if Window variable exists and has CreateTab (this will create a duplicate Main if used wrongly)
    if typeof(Window) == 'table' and Window.CreateTab then
        -- find existing tab by name (Rayfield doesn't expose a getter), so best approach is to create a new tab named "Main (Extras)" and keep it minimal.
        local TargetTab = Window:CreateTab("Main")
...
-- Touch Sensitivity (Rayfield slider) - attached to Main tab bottom
if type(Rayfield) == "table" and MainTab then
...
elseif type(Rayfield) == "table" then
    -- fallback: create a small tab named "Main Extras" and add the slider there
    local FallbackTab = Window:CreateTab("Main Extras", 4483362458)
    FallbackTab:CreateSlider({
        Name = "Touch Sensitivity",
        Range = {CONFIG.MIN_SENSITIVITY, CONFIG.MAX_SENSITIVITY},
        Increment = 0.1,
        Suffix = "",
