...
-- Dash Assist with Front/Back modes, yaw-only aimlock, animations, sounds, mobile+PC+controller support
-- ESP highlights the chosen target (nearest if "Find nearest" is ON, or the manually selected player).
...
-- Unified smooth arc function supporting Front/Back direction
...
local function createUI()
    ...
    -- Direction segmented control (Back / Front)
    ...
    dashBtn.Size = UDim2.new(0,120,0,120)
    dashBtn.Position = UDim2.new(0.5,-60,1,-140)
    dashBtn.AnchorPoint = Vector2.new(0.5,0)
    dashBtn.BackgroundColor3 = Color3.fromRGB(30,30,30)
    dashBtn.BorderSizePixel = 0
    dashBtn.Image = "rbxassetid://99317918824094"
    dashBtn.Parent = mainFrame
    local dashCorner = Instance.new("UICorner", dashBtn)
    dashCorner.CornerRadius = UDim.new(0,18)

    local uiScale = Instance.new("UIScale")
    uiScale.Scale = 1
    uiScale.Parent = dashBtn
    ...
    local function startPointer(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            buttonStartPos = dashBtn.Position
            trackedInput = input
    ...
    local function updatePointer(input)
        ...
        if isDragging then
            ...
            newX = math.clamp(newX,0,screenW - dashBtn.AbsoluteSize.X)
            newY = math.clamp(newY,0,screenH - dashBtn.AbsoluteSize.Y)
            dashBtn.Position = UDim2.new(0,newX,0,newY)
        end
    end
    ...
    UserInputService.InputEnded:Connect(function(input)
        ...
        if not isDragging and not busy then
            -- activation: respect the ACTIVATION_RANGE and findNearestEnabled/selection
            ...
            if chosenCharacter then
                smoothArcTo(chosenCharacter, directionMode)
            end
        end
        tweenUIScale(1,0.06)
    ...
    dashBtn.InputBegan:Connect(function(input) pcall(function() startPointer(input) end) end)
...
UserInputService.InputBegan:Connect(function(input,processed)
    ...
    if activated then
        ...
        if chosenCharacter then smoothArcTo(chosenCharacter, directionMode) end
    end
end)

print("[CircularTweenUI] Ready - mobile button + X keybind + Gamepad DPadUp active.")
print("[CircularTweenUI] Activation range: " .. ACTIVATION_RANGE .. " studs. Arc approach radius: " .. ARC_APPROACH_RADIUS .. " studs.")
