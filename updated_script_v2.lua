-- Merged script — modified per user request (v2):
-- Key changes in this version:
--  * Replaced instant final CFrame teleport with a short "settle" velocity phase so the player actually moves to the final spot (no teleport).
--  * Reduced ARC_APPROACH_RADIUS default from 9 -> 8 as requested.
--  * Ensures side dash to front or back torso with smoother movement (still uses AssemblyLinearVelocity to avoid sudden teleports).
--  * Other UI values reflect the new default.
...
local MAX_RANGE = 30
local ARC_APPROACH_RADIUS = 8 -- changed from 9 -> 8
...
local function smoothArcToTarget(targetModel)
    ...
    -- reduce vertical lift to help avoid floating issues
    ...
    conn = RunService.Heartbeat:Connect(function(delta)
        ...
        -- Orientation yaw calculation (for facing direction)
        ...
        -- Compute velocity needed to reach posNow this frame
        ...
        -- update rotation only (don't teleport position) to avoid jitter
        pcall(function()
            if HRP and HRP.Parent then
        ...
        if t >= 1 then
            if conn and conn.Connected then conn:Disconnect() end
            -- Instead of teleporting, perform a short "settle" phase using velocity so the character actually moves to the final spot.
            -- This prevents instant teleport while still ensuring we end up at the desired finalPos.
            local settleDur = 0.14
            local settleStart = tick()
            local settlePrev = tick()
            while tick() - settleStart < settleDur do
                if not HRP or not HRP.Parent then break end
                local now2 = tick()
                local dt2 = math.max(0.0001, now2 - settlePrev)
                settlePrev = now2
                -- velocity proportional to remaining distance (divided by small time window for snappy settle)
                local remaining = finalPos - HRP.Position
                local desired = remaining / math.max(0.0001, math.min(0.06, settleDur - (now2 - settleStart)))
                pcall(function()
                    if HRP and HRP.Parent then
                        HRP.AssemblyLinearVelocity = desired
                        -- keep facing desired direction without teleporting
                        HRP.CFrame = CFrame.new(HRP.Position, HRP.Position + Vector3.new(math.cos(currentDesiredHRPYaw), 0, math.sin(currentDesiredHRPYaw)))
                    end
                end)
                RunService.Heartbeat:Wait()
            end
            -- final cleanup: stop velocity (do not snap)
            pcall(function()
                if HRP and HRP.Parent then
...
-- UI and other unchanged parts below (only defaults and delays were updated earlier)...
...
local function createExtraFrame()
    ...
    studsPlus.MouseButton1Click:Connect(function()
        -- increase SPEED_SETTING by 1, max 40
        ...
        if not isDragging and not busy then
            ...
            if targetModel then
                ...
                if ok and started then
                    pcall(function() pressSFX:Play() end)
                    -- Changed M1 delay to 0.15 per request
                    if m1Enabled then
...
print("[Merged v2] Circular velocity-based movement + Settings updated and ready.")

-- Watermark UI (unchanged)
do
    ...
    -- (rest of watermark omitted for brevity in this file version)
end
