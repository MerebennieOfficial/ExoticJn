...
-- Helper: replace tween movement with LinearVelocity-based movement
local function moveWithLinearVelocity(part, targetCFrame, dur)
	if not part or not part:IsA("BasePart") or not targetCFrame or not dur or dur <= 0 then return end
	-- Ensure attachment exists
	local attachment = part:FindFirstChild("AutoMoveAttachment") or Instance.new("Attachment")
	attachment.Name = "AutoMoveAttachment"
	if attachment.Parent ~= part then
		attachment.Parent = part
	end

	-- Create LinearVelocity
	local lv = Instance.new("LinearVelocity")
	lv.Name = "AutoMoveLinearVelocity"
	lv.Attachment0 = attachment
	-- Use a very large max force so it can move the character reliably in executors
	lv.MaxForce = Vector3.new(1e8, 1e8, 1e8)
	-- Calculate velocity vector to reach target in 'dur' seconds
	local velocity = (targetCFrame.Position - part.Position) / dur
	lv.VectorVelocity = velocity
	lv.RelativeTo = Enum.ActuatorRelativeTo.World
	lv.Parent = part
	lv.Enabled = true

	-- After duration, snap to exact CFrame and clean up
	task.spawn(function()
		task.wait(dur)
		-- Snap to final CFrame to avoid small differences
		pcall(function()
			part.CFrame = targetCFrame
		end)
		-- Disable and destroy the linear velocity
		if lv then
			lv.Enabled = false
			lv:Destroy()
		end
		-- Optionally remove the attachment if it's our created one
		if attachment and attachment.Name == "AutoMoveAttachment" and attachment.Parent == part then
			-- keep the attachment to reuse later (avoid flood), so do not destroy
		end
	end)
end
							...
							while noClip == false do
								...
								while m do
									...
									if target ~= nil and target == dummy then
										...
										-- replaced tween with LinearVelocity move
										...
										task.spawn(function()
											moveWithLinearVelocity(p, ps.CFrame, duration)
										end)
									end
