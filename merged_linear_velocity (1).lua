-- Merged Circular Movement using AssemblyLinearVelocity (no TweenService)
-- Replaced tween logic with Linear Velocity and aimlock that ignores camera.
-- Full script ready to drop into a LocalScript.
...
local TOTAL_TIME = 0.3          -- base duration (will be divided by SPEED_SETTING)
...
-- Smooth arc using AssemblyLinearVelocity (no TweenService).
-- We'll compute the desired arc position per-frame and set HRP.AssemblyLinearVelocity
local function smoothArcToTargetLinearVelocity(targetModel)
    ...
    -- Save auto-rotate and disable it (we'll control orientation).
    local humanoid = nil
    ...
    -- We'll run a Heartbeat loop to set AssemblyLinearVelocity each frame.
    local conn
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Orientation: make character face the target using look vector from HRP to target.
        -- This logic ignores the camera (aimlock behavior user requested).
        if HRP and HRP.Parent then
            local lookVectorToTarget = (targetHRP.Position - HRP.Position)
            if lookVectorToTarget.Magnitude > 0.0001 then
                local lv = lookVectorToTarget.Unit
                HRP.CFrame = CFrame.new(HRP.Position, HRP.Position + lv)
            end
        end

        -- Compute desired velocity to reach posNow in this frame.
        local deltaPos = posNow - HRP.Position
        local desiredVel
        if dt > 0 then
            desiredVel = deltaPos / dt
        else
            desiredVel = Vector3.new(0,0,0)
        end

        -- Apply to AssemblyLinearVelocity (physics-driven movement)
        pcall(function()
        ...
        -- If finished, set final position and cleanup
        if t >= 1 then
            if conn and conn.Connected then conn:Disconnect() end
            ...
            pcall(function()
                if HRP and HRP.Parent then
                    HRP.CFrame = CFrame.new(finalPos, finalPos + Vector3.new(math.cos(finalYaw), 0, math.sin(finalYaw)))
                    -- Zero out velocity to stop motion
...
-- UI (no TweenService). Minimal instant UI - behavior kept but animations removed.
...
-- MINIMIZE / OPEN ANIMS (instant open/close, no tween)
...
local function createDashButton()
    ...
    -- Basic pointer/drag behavior (no tweens)
    ...
    UserInputService.InputEnded:Connect(function(input)
        ...
        if not isDragging and not busy then
            ...
            if targetModel then
                local started = false
                local ok, err = pcall(function() started = smoothArcToTargetLinearVelocity(targetModel) end)
                if ok and started then
                    pcall(function() pressSFX:Play() end)
                    -- 0.1 second delay before M1 activates
                    if m1Enabled then
...
print("[Merged LinearVelocity] Circular movement (no TweenService) loaded and ready.")

-- Optional: simple watermark (instant appearance, no tween)
do
    ...
    frame.Size = UDim2.new(0, 330, 0, 72)
    frame.AnchorPoint = Vector2.new(1, 0)
    frame.Position = UDim2.new(1, 10, 0, 18)
    ...
    clickSound2.Name = "ClickSound"
    clickSound2.SoundId = "rbxassetid://142376088"
    clickSound2.Volume = 1
    clickSound2.Parent = frame

    clickBtn.MouseButton1Click:Connect(function()
        pcall(function() clickSound2:Play() end)
        local ok
        ok = pcall(function() if setclipboard then setclipboard(DISCORD_LINK) end end)
        if not ok then
            ok = pcall(function() if syn and syn.set_clipboard then syn.set_clipboard(DISCORD_LINK) end end)
        end
    end)
end
