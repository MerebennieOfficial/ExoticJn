...
-- Dragging mainFrame (fixed: use AbsolutePosition so vertical movement works on mobile)
    ...
    topBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- use AbsolutePosition for robust pixel-based dragging
            startAbsPos = Vector2.new(mainFrame.AbsolutePosition.X, mainFrame.AbsolutePosition.Y)

            -- Ensure we stop dragging on input end
    ...
    topBar.InputChanged:Connect(function(input)
        -- listen for movement from touch or mouse movement
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        -- clamp using AbsoluteSize for accurate bounds
