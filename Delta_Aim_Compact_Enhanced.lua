...
--[[
UI & Feature Enhancements by ChatGPT for Merebennie
Changes:
- Compact UI preserved from original.
- Solid background with white text + thin black outline.
- Moveable UI (touch + mouse).
- New button click sound (rbxassetid://6042053626).
- Pixel-style "Made by Merebennie" watermark in original position.
- Extra features integrated without breaking original functions.
]]

local BUTTON_CLICK_SOUND = "rbxassetid://6042053626"

-- Function to style text (white with thin black outline)
local function styleTextWithOutline(obj)
...
-- Apply moveable behavior to a frame
...
-- Add watermark
local function addWatermark(gui)
    local wm = Instance.new("TextLabel")
    wm.Size = UDim2.new(0,200,0,20)
    wm.Position = UDim2.new(0,10,0,10) -- same as original placement
    wm.BackgroundTransparency = 1
    wm.Font = Enum.Font.Arcade
    wm.TextSize = 16
    wm.Text = "Made by Merebennie"
    styleTextWithOutline(wm)
    wm.Parent = gui
end

-- Example: Hook into your UI creation (after ScreenGui is created in original code)
--[[
local gui = Instance.new("ScreenGui")
gui.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")
addWatermark(gui)
makeDraggable(mainFrame) -- mainFrame from your original script
]]
...
-- Watermark (top center, small, thin black outline)
local watermark = Instance.new("TextLabel")
watermark.Name = "Watermark"
watermark.Parent = gui
watermark.AnchorPoint = Vector2.new(0.5, 0)
watermark.Position = UDim2.new(0.5, 0, 0, 6) -- top center
...
title.TextColor3 = Color3.new(1,1,1); title.Text = "Pengontrol Aimlock"; title.TextXAlignment = Enum.TextXAlignment.Left
...
settingsBtn.Text = "⚙"; settingsBtn.Font = Enum.Font.GothamBold; settingsBtn.TextSize = 18
...
toggleBtn.Text = "AIMBOT: NONAKTIF"; toggleBtn.BackgroundColor3 = Color3.fromRGB(50,50,50)
...
-- Factories with adjusted slider button positions
...
-- slider (labels translated to Indonesian)
...
local function rowToggle(label, default, callback)
    local v = default
    local btn = rowButton(label .. ": " .. (v and "AKTIF" or "NONAKTIF"))
    btn.MouseButton1Click:Connect(function()
        v = not v
        btn.Text = label .. ": " .. (v and "AKTIF" or "NONAKTIF")
        pcall(callback, v)
    end)
...
-- dropdown that accepts label/value pairs (display label translated, value kept for logic)
...
-- Quick movable button (translated)
...
-- SETTINGS ROWS (translated labels and options)
-- Aim smoothing
rowSlider("Kehalusan Aim (0..1)", 0, 1, 0.05, CFG.SMOOTHING, function(v) CFG.SMOOTHING = v end)

-- Aim mode dropdown
rowDropdown("Mode Aim", { {label="Halus", value="Smooth"}, {label="Snap", value="Snap"} }, CFG.AIM_MODE, function(v) CFG.AIM_MODE = v end)

-- Use prediction toggle
rowToggle("Gunakan Prediksi", CFG.USE_PREDICTION, function(v) CFG.USE_PREDICTION = v end)

-- Bullet speed slider
rowSlider("Kecepatan Peluru (prediksi)", 200, 5000, 50, CFG.BULLET_SPEED, function(v) CFG.BULLET_SPEED = v end)

-- Prediction multiplier
rowSlider("Pengali Prediksi", 0, 3, 0.05, CFG.PREDICT_MULT, function(v) CFG.PREDICT_MULT = v end)

-- Target priority dropdown
rowDropdown("Prioritas Target", { {label="Sudut", value="Angle"}, {label="Layar", value="Screen"}, {label="Jarak", value="Distance"} }, CFG.TARGET_PRIORITY, function(v) CFG.TARGET_PRIORITY = v end)

-- Switch mode dropdown
rowDropdown("Mode Ganti", { {label="Dengan Pandangan", value="ByLook"}, {label="Terdekat", value="Closest"} }, "ByLook", function(v) if v then CFG.SWITCH_MODE = v end end)

-- ADDED: Tilt Layar slider (visible and translated)
rowSlider("Tilt Layar (derajat)", -15, 15, 1, CFG.SCREEN_TILT, function(v) CFG.SCREEN_TILT = v end)

-- Switch sensitivity (yaw & pitch)
rowSlider("Sensitivitas Ganti (yaw)", 0.001, 0.02, 0.001, CFG.SWITCH_SENSITIVITY_YAW, function(v) CFG.SWITCH_SENSITIVITY_YAW = v end)
rowSlider("Sensitivitas Ganti (pitch)", 0.005, 0.12, 0.005, CFG.SWITCH_SENSITIVITY_PITCH, function(v) CFG.SWITCH_SENSITIVITY_PITCH = v end)

-- Switch cooldown
rowSlider("Cooldown Ganti (s)", 0.02, 0.5, 0.01, CFG.SWITCH_COOLDOWN, function(v) CFG.SWITCH_COOLDOWN = math.max(0.02, v) end)

-- FOV size
rowSlider("Ukuran FOV (px)", 20, 400, 5, CFG.FOV_PIXELS, function(v) CFG.FOV_PIXELS = v; if fovFrame then fovFrame.Size = UDim2.new(0, v*2, 0, v*2) end end)

-- Show FOV
rowToggle("Tampilkan Cincin FOV", CFG.SHOW_FOV, function(v) CFG.SHOW_FOV = v; if fovFrame then fovFrame.Visible = v end end)

-- Friend filter
rowToggle("Filter Teman", CFG.USE_FRIEND_FILTER, function(v) CFG.USE_FRIEND_FILTER = v end)

-- ESP settings (translated)
rowToggle("Aktifkan ESP", CFG.ESP_ENABLED, function(v)
    CFG.ESP_ENABLED = v
    if v then
...
rowToggle("ESP Tembus Dinding (SelaluDiAtas)", CFG.ESP_ALWAYS_ON_TOP, function(v)
...
rowToggle("Tampilkan Nama", CFG.ESP_SHOW_NAME, function(v)
    CFG.ESP_SHOW_NAME = v
    for _,plr in ipairs(Players:GetPlayers()) do
        local data = _ESP and _ESP[plr]
        if data then
    ...
    local function buildForCharacter(char)
        ...
        if head then
            local ok2, billboard = pcall(function()
                ...
                -- thin border behind text to improve readability (Frame)
