...
-- Utility to move a part towards a target CFrame using LinearVelocity.
-- This replaces the previous TweenService usage.
local function applyLinearVelocityToPart(part, targetCFrame, time)
	if not part or not targetCFrame or not time then return end
	-- Ensure there's an attachment at the part's center for the constraint
	local attach = part:FindFirstChild("__lv_attach")
	if not attach then
		attach = Instance.new("Attachment")
		attach.Name = "__lv_attach"
	...
	lv.Name = "__lv_constraint"
	lv.Attachment0 = attach
	lv.Attachment1 = nil
	lv.VelocityConstraintMode = Enum.VelocityConstraintMode.Vector
	lv.MaxForce = math.huge
	-- compute velocity required to reach target in `time` seconds
	local displacement = (targetCFrame.Position - part.Position)
	local t = math.max(time, 0.0001)
	lv.VectorVelocity = displacement / t
	lv.Parent = part
	lv.Enabled = true

	-- Cleanup after the movement duration (small buffer)
	game:GetService("Debris"):AddItem(lv, t + 0.05)
	-- keep attachment (may be reused), but remove it if you prefer:
	-- game:GetService("Debris"):AddItem(attach, t + 0.05)
end
							...
							while noClip == false do
								...
								while m do
									...
									if target ~= nil and target == dummy then
										...
										-- replaced TweenService tween with LinearVelocity movement
										applyLinearVelocityToPart(p, ps.CFrame, speedN)
										task.delay(duration,function()
											m = false
										end)
