-- InstantTwisted (improved) + cooldown bar UI
-- Adds a small green-to-empty cooldown bar above your character when the configured cooldown animation triggers.
-- All original behavior preserved; cooldown still only activates when COOLDOWN_TRIGGER_ANIM is detected.
...
local POST_DETECT_DELAY = 0.3
local FIRST_ROT_DEG = 200
local SECOND_ROT_DEG = 160
local BETWEEN_ROT_DELAY = 0.05

local MOVE_DISTANCE = 3
local MOVE_DURATION = 0.08
local EXTRA_ROT_DELAY = 0.35 -- changed from 0.44 to 0.3
local EXTRA_ROT_DEGREES = 350
local EXTRA_ROT_DISABLE_TIME = 0.25
-- cooldown & proximity
local COOLDOWN_TIME = 4
local PROXIMITY_RANGE = 8

-- New: which animation should trigger the cooldown (from your provided id)
local COOLDOWN_TRIGGER_ANIM = "10479335397" -- corresponds to rbxassetid://10479335397

-- internal cooldown bookkeeping
local _cooldown = false
local _cooldownStart = nil
local _cooldownEnd = nil

-- Cooldown UI handles (one per local character)
local cooldownGuiWrapper = nil -- { Gui = BillboardGui, Outer = Frame, Fill = Frame, token = {cancel=false} }
...
-- performRemotes replaced with your exact provided pattern but wrapped in pcall for safety
local function performRemotes()
    pcall(function()
        ...
        -- try preferred safe path first
        local ok, char = pcall(getCharacter)
        if ok and char and char:FindFirstChild("Communicate") and type(char.Communicate.FireServer) == "function" then
            pcall(function() char.Communicate:FireServer(unpack(args)) end)
            return
        end
        -- fallback: try LocalPlayer.Character path (safe)
        pcall(function()
            if Players and Players.LocalPlayer and Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("Communicate") then
                Players.LocalPlayer.Character.Communicate:FireServer(unpack(args))
            end
        end)
    end)
end
...
-- CHANGED: pushBackwardForce (was pushForwardForce) to move BACKWARD instead of forward
local function pushBackwardForce(root, humanoid, distance, duration)
    if not root then return end
    ...
    pcall(function()
        root.AssemblyLinearVelocity = backDir * speed
    end)

    pcall(function()
        if humanoid and humanoid.Move then
            humanoid:Move(backDir, true)
        elseif humanoid and typeof(humanoid.Move) == "function" then
            humanoid:Move(backDir)
        end
    end)
    ...
    pcall(function()
        ...
        for i = 1, tries do
            ...
            pcall(function() root.CFrame = root.CFrame + backDir * per end)
...
-- CORE SEQUENCE (keeps original cooldown gating logic - cooldown variable checked at start)
local function doSequence()
    ...
    -- NOTE: cooldown is set when the configured animation is detected (see handleTrack).
    ...
    -- SECOND rotation: rotate, then push backward
    rotateYawRelative(root, SECOND_ROT_DEG)

    -- CALL THE BACKWARD PUSH at the second rotate (activated now)
    pcall(function()
        pushBackwardForce(root, humanoid, MOVE_DISTANCE, MOVE_DURATION)
    end)
    ...
    -- NOTE: Extra 350° rotation block removed as requested.
...
-- --- Cooldown UI utility functions ------------------------------------------------
local function destroyCooldownGui()
    if cooldownGuiWrapper then
        -- cancel running updater
        pcall(function() if cooldownGuiWrapper.token then cooldownGuiWrapper.token.cancel = true end end)
        pcall(function() if cooldownGuiWrapper.Gui and cooldownGuiWrapper.Gui.Parent then cooldownGuiWrapper.Gui:Destroy() end end)
        cooldownGuiWrapper = nil
    end
end

local function createCooldownGuiForCharacter(char)
    -- destroy old
    destroyCooldownGui()
    if not char then return end
    -- find root to adorn
    local root = char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or char:FindFirstChild("Head")
    if not root then return end
    -- prefer PlayerGui parent and set Adornee to root so it follows the character
    local playerGui = nil
    pcall(function() playerGui = LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui") end)
    if not playerGui then
        -- fallback, try waiting briefly
        playerGui = LocalPlayer:WaitForChild("PlayerGui", 1)
    end
    if not playerGui then return end

    local bgui = Instance.new("BillboardGui")
    bgui.Name = "InstantTwistedCooldownGui"
    bgui.Adornee = root
    bgui.AlwaysOnTop = true
    bgui.Size = UDim2.new(0, 120, 0, 12)
    bgui.StudsOffset = Vector3.new(0, 3.2, 0) -- above head
    bgui.MaxDistance = 100
    bgui.Parent = playerGui
    bgui.Enabled = false

    -- outer black outline (small border)
    ...
    -- inner green fill (anchored left)
    local fillMask = Instance.new("Frame")
    fillMask.Name = "FillMask"
    fillMask.Size = UDim2.new(1, -4, 1, -4)
    fillMask.Position = UDim2.new(0, 2, 0, 2)
    fillMask.BackgroundTransparency = 1
    fillMask.BorderSizePixel = 0
    fillMask.Parent = outer

    local fill = Instance.new("Frame")
    fill.Name = "Fill"
    fill.Size = UDim2.new(1, 0, 1, 0) -- full initially
    fill.Position = UDim2.new(0, 0, 0, 0)
    fill.AnchorPoint = Vector2.new(0, 0)
    fill.BackgroundColor3 = Color3.fromRGB(94, 189, 82) -- green
    fill.BorderSizePixel = 0
    fill.Parent = fillMask

    local fillCorner = Instance.new("UICorner")
    fillCorner.CornerRadius = UDim.new(0, 3)
    fillCorner.Parent = fill

    cooldownGuiWrapper = { Gui = bgui, Outer = outer, Fill = fill, token = nil }
    return cooldownGuiWrapper
end

local function showCooldownBar(startTime, duration)
    if not cooldownGuiWrapper then
        -- attempt to (re)create for current character
        local char = getCharacter()
        if char then createCooldownGuiForCharacter(char) end
    end
    if not cooldownGuiWrapper then return end

    -- cancel previous updater if running
    if cooldownGuiWrapper.token then cooldownGuiWrapper.token.cancel = true end
    local token = { cancel = false }
    cooldownGuiWrapper.token = token

    local gui = cooldownGuiWrapper.Gui
    local fill = cooldownGuiWrapper.Fill
    if not gui or not fill then return end
    gui.Enabled = true

    local tEnd = startTime + duration
    -- update loop: shrink from full (1) to empty (0)
    spawn(function()
        while not token.cancel do
            local now = tick()
            local remaining = math.clamp(tEnd - now, 0, duration)
            local progress = 0
            if duration > 0 then
                progress = remaining / duration -- 1 -> 0
            end
            -- ensure fill anchored left and width corresponds to progress
            pcall(function()
                fill.Size = UDim2.new(progress, 0, 1, 0)
            end)
            if now >= tEnd then break end
            task.wait(0.03)
        end
        -- end state
        pcall(function() fill.Size = UDim2.new(0, 0, 1, 0) end)
        -- small delay to let player notice empty, then hide
        task.wait(0.08)
...
-- When character spawns, create cooldown GUI and ensure it's recreated on respawn.
local function setupCooldownGuiForCharacter(char)
    destroyCooldownGui()
    createCooldownGuiForCharacter(char)
end
...
local function handleTrack(track)
    ...
    -- Start the cooldown only when the configured cooldown-trigger animation is detected
    if num == COOLDOWN_TRIGGER_ANIM then
        -- only set cooldown if it's not already active
        if not _cooldown then
            _cooldown = true
            _cooldownStart = tick()
            _cooldownEnd = _cooldownStart + COOLDOWN_TIME
            -- show UI bar above character (from full -> empty)
            pcall(function() showCooldownBar(_cooldownStart, COOLDOWN_TIME) end)
            -- schedule reset of the cooldown flag
...
local function onCharacterAdded(char)
    ...
    -- setup cooldown GUI for this character so it appears above the head and reappears after respawn
    pcall(function() setupCooldownGuiForCharacter(char) end)
end
...
LocalPlayer.CharacterRemoving:Connect(function(char)
    clearHumanoidBindings()
    -- destroy cooldown GUI on removal
    pcall(function() destroyCooldownGui() end)
end)
...
print("InstantTwisted (improved) loaded. Movable UI created. Animation", ANIM_ID_PREFIX, "will trigger the sequence and will rebind after respawn. Cooldown bar UI enabled for", COOLDOWN_TRIGGER_ANIM)
