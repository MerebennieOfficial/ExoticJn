...
local function stickDash()
    ...
    startImmobilize()
    -- fireQ removed from here so dash only fires after the 0.3s detection delay (see onAnim)
...
local function onAnim(track)
    ...
    if anim then
        ...
        -- If one of the specific animation IDs plays, wait 0.3s, then fire dash instantly,
        -- and then trigger the stick/dash routine (tryTrigger).
        if string.find(idStr, "10503381238", 1, true) then
            task.delay(0.3, function()
                pcall(fireQ) -- fire dash after 0.3s
                tryTrigger()
            end)
            return
        end
