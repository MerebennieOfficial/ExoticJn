-- Supa Tech V2 - Optimized for PC (reduced FPS drops)
-- Key optimizations:
-- 1) Replaced expensive workspace:GetDescendants() search with Players:GetPlayers() iteration.
-- 2) Reduced per-frame work: avoid constantly setting HRP.CFrame in a Heartbeat loop.
-- 3) Replaced tight Heartbeat "anti" connection with a lower-frequency loop to zero velocity.
-- 4) Properly Disconnect() connections and avoid creating many short-lived Heartbeat connections.
-- 5) Minor code cleanup and comments.
...
-- Efficient nearest target search using Players list instead of workspace:GetDescendants()
    ...
    for _, pl in pairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer and pl.Character and pl.Character:FindFirstChild("HumanoidRootPart") then
            local ok, mag = pcall(function()
                return (HumanoidRootPart.Position - pl.Character.HumanoidRootPart.Position).Magnitude
            end)
...
local function stickDash()
    ...
    -- We'll use a lighter-weight immobilize approach (not every-frame Heartbeat)
    local stopImmobilizeFlag = false
    local antiThread
    local function startImmobilize()
        ...
        -- Destroy obvious physics movers once to avoid conflicts (this is usually quick)
        ...
        -- Lower-frequency loop to keep player stopped but avoid heavy per-frame operations
        stopImmobilizeFlag = false
        antiThread = task.spawn(function()
            while not stopImmobilizeFlag do
                ...
                task.wait(0.06) -- ~16 checks a second (reduce load compared to every-frame)
            end
        end)
    end

    local function stopImmobilize()
        stopImmobilizeFlag = true
        if antiThread then
            -- antiThread will self-stop due to flag; wait a tiny bit to let it exit cleanly
            task.wait(0.01)
            antiThread = nil
        end
        pcall(function()
    ...
    -- Change state and move using LinearVelocity to avoid directly setting CFrame (smoother physics)
    pcall(function() Humanoid:ChangeState(Enum.HumanoidStateType.Physics) end)
    if HumanoidRootPart then
        local ok, targetPos = pcall(function()
            return (targetHRP.Position - targetHRP.CFrame.LookVector * 0.3)
        end)
        if ok and targetPos then
            pcall(function()
                -- Create or reuse an Attachment on the HRP
                local att = HumanoidRootPart:FindFirstChild("Supa_LV_Att")
                if not att then
                    att = Instance.new("Attachment")
                    att.Name = "Supa_LV_Att"
                    att.Parent = HumanoidRootPart
                    att.Position = Vector3.new(0,0,0)
                end
                -- Clean up any previous LinearVelocity we created
                local prev = HumanoidRootPart:FindFirstChild("Supa_LinearVelocity")
                if prev then
                    pcall(function() prev:Destroy() end)
                end
                -- Create LinearVelocity to move the HRP toward the target position quickly
                local lv = Instance.new("LinearVelocity")
                lv.Name = "Supa_LinearVelocity"
                lv.Attachment0 = att
                lv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                -- Compute a velocity that reaches the position in ~0.12s (clamped to avoid absurd values)
                local moveTime = 0.12
                local delta = (targetPos - (HumanoidRootPart.Position or Vector3.new()))
                local vel = delta / math.max(moveTime, 0.05)
                lv.Velocity = vel
                lv.Parent = HumanoidRootPart
                -- Remove the LinearVelocity shortly after to release control back to physics
    ...
    -- Small delay to mimic the original layDuration but without heavy per-frame work
    task.wait(0.2)
...
-- GUI creation (unchanged except for some cleanup)
