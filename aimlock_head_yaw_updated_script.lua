...
local function smoothArcToTarget(targetModel)
    ...
    conn = RunService.Heartbeat:Connect(function(delta)
        ...
        -- Faster yaw-only aimlock targeting the target's Head when available
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            pcall(function()
                local camPos = cam.CFrame.Position
                -- prefer head if present
                local head = nil
                if targetModel and targetModel.FindFirstChild then
                    head = targetModel:FindFirstChild(\"Head\")
                end
                local aimPos = nil
                ...
                -- keep camera's vertical (pitch) unchanged by projecting target onto camera Y plane
                local desiredFlat = Vector3.new(aimPos.X, camPos.Y, aimPos.Z)
                -- compute current and desired yaw
                local currentLook = cam.CFrame.LookVector
                local currentYaw = math.atan2(currentLook.Z, currentLook.X)
                local desiredYaw = math.atan2(desiredFlat.Z - camPos.Z, desiredFlat.X - camPos.X)
                local yawDelta = shortestAngleDelta(desiredYaw, currentYaw)
                -- interpolate yaw quickly (factor tuned for 'a bit faster')
                local interpFactor = math.clamp(dt * 120, 0, 1) -- very responsive; adjust multiplier if needed
                local newYaw = currentYaw + yawDelta * interpFactor
                cam.CFrame = CFrame.new(camPos, camPos + Vector3.new(math.cos(newYaw), 0, math.sin(newYaw)))
            end)
        end
        ...
        if t >= 1 then
            ...
            pcall(function()
                if HRP and HRP.Parent then
                    ...
                    -- determine final facing from target's current look vector (yaw-only)
                    local finalLook = nil
                    if targetHRP and targetHRP.Parent then
                        finalLook = targetHRP.CFrame.LookVector
                    else
                        finalLook = Vector3.new(math.cos(currentDesiredHRPYaw), 0, math.sin(currentDesiredHRPYaw))
                    end
                    HRP.CFrame = CFrame.new(posNow, posNow + Vector3.new(finalLook.X, 0, finalLook.Z))
                end
            end)
