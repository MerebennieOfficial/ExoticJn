...
-- Helper: detect ragdoll state (prevents activation while ragdolled)
local function isRagdolled()
    ...
    local rv = Character:FindFirstChild("Ragdoll")
...
-- ---------- Slider mapping helpers (moved up so UI can show mapped values) ----------
local function sliderToTotalTime(percent)
    -- New mapping: percent 0..100 -> 1.5s (slow) .. 0.12s (very fast)
    local p = math.clamp(percent or 84, 0, 100) / 100
    -- use linear interpolation; user asked for more significance so min time decreased
    local slow = 1.5
    local fast = 0.12
    return slow + (fast - slow) * p
end
local function sliderToDegrees(percent)
    -- percent: 0..100 -> 90..1080 degrees (wider range for more dramatic effect)
    local p = math.clamp(percent or 56, 0, 100) / 100
    return 90 + (1080 - 90) * p
end
local function sliderToOrbitRadius(percent)
    -- percent: 0..100 -> 1..12 studs
    local p = math.clamp(percent or 50, 0, 100) / 100
    return 1 + (12 - 1) * p
end


-- Choose target respecting selectedPlayer and MAX_RANGE; do NOT clear selection if out of range.
-- If a player is selected but out-of-range, return nil so activation does nothing.
local selectedPlayer = nil

-- Clear selection if the player leaves the game
Players.PlayerRemoving:Connect(function(plr)
    if selectedPlayer == plr then selectedPlayer = nil end
end)

local function chooseTarget()
    -- If a player was explicitly selected, only return them if valid and within range.
    if selectedPlayer then
        if selectedPlayer.Character and selectedPlayer.Character.Parent then
            local ch = selectedPlayer.Character
            local hrp = ch:FindFirstChild("HumanoidRootPart")
            local hum = ch:FindFirstChild("Humanoid")
            if hrp and hum and hum.Health > 0 and HRP then
                local d = (hrp.Position - HRP.Position).Magnitude
                if d <= MAX_RANGE then
                    return ch
                else
                    -- IMPORTANT: do NOT clear selectedPlayer here.
                    -- Return nil to indicate no valid activation target (user selected someone but they're out of range).
                    return nil
                end
            else
                -- selected player's character invalid/dead; clear selection so next activation can fallback.
                selectedPlayer = nil
            end
        else
            -- selected player object invalid; clear selection.
            selectedPlayer = nil
        end
    end
    -- No explicit selection -> fallback to nearest (players + NPCs) within MAX_RANGE
    local nearest, _ = getNearestTarget(MAX_RANGE)
    return nearest
end
end)
local function chooseTarget()
        ...
        if selectedPlayer.Character and selectedPlayer.Character.Parent then
            ...
            if hrp and hum and hum.Health > 0 and HRP then
                ...
                if d <= MAX_RANGE then
                    ...
                    -- selected player is out of range; clear selection and fall back
                    selectedPlayer = nil
                end
            else
                -- selected player's character invalid/dead; clear selection
                selectedPlayer = nil
            end
        else
            -- selected player object invalid; clear
            selectedPlayer = nil
        end
    end
    local nearest, _ = getNearestTarget(MAX_RANGE)
    ...
    if STRAIGHT_ANIM_ID then
        ...
        if hum and animator then
            ...
            if ok and track then
                ...
                pcall(function() straightAnimTrack.Looped = false end)
...
-- MAIN SETTINGS FRAME (wider to fit indicators)
...
{Size = UDim2.new(0, 340, 0, 270)}):Play()
...
local function refreshPlayers()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            ...
            -- clicking selects this player as target (if in range when activating)
            btn.MouseButton1Click:Connect(function()
                selectedPlayer = plr
                -- highlight selected button, clear others
                for _,c in ipairs(playerList:GetChildren()) do if c:IsA("TextButton") then c.BackgroundColor3 = Color3.fromRGB(235,235,235) end end
            ...
            -- highlight if this player is currently selected
            if selectedPlayer == plr then
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                -- toggle selection: clicking the same player again will deselect
                if selectedPlayer == plr then
                    selectedPlayer = nil
                    btn.BackgroundColor3 = Color3.fromRGB(235, 235, 235)
                else
...
-- Ensure selection cleared if that player leaves
...
local function createDashPanel()
    ...
    CloseButton.MouseButton1Click:Connect(function() end) -- on-screen X removed (keyboard X still works)
    ...
    for i, name in ipairs(TitleLabels) do
        ...
        SliderButton.Size = UDim2.new(0, 20, 0, 20) -- bigger knob
        ...
        -- Indicator to show mapped value (right side)
        local Indicator = Instance.new("TextLabel")
        Indicator.Parent = MainFrame
        Indicator.Size = UDim2.new(0, 70, 0, 20)
        Indicator.Position = UDim2.new(0.82, 0, 0.18 + (i - 1) * 0.24, 0)
        Indicator.BackgroundTransparency = 1
        Indicator.Font = Enum.Font.GothamBold
        Indicator.TextColor3 = Color3.fromRGB(25,25,25)
        ...
        if currentSliderValues[name] ~= nil then
            ...
            if name == "Dash speed" then
                -- default mapped to ~0.82s (user requested)
                initValue = 49 -- ~49% maps to ~0.82s
            elseif name == "Dash Degrees" then
                -- default to ~407 degrees (user requested)
                initValue = math.floor((407 - 90) / (1080 - 90) * 100)
            elseif name == "Dash gap" then
                -- default to ~2.5 studs
                initValue = 14
            end
        end
        value = initValue
        SliderButton.Position = UDim2.new(value / 100, -10, 0.5, -10)
        currentSliderValues[name] = value

        local function updateIndicator()
            if name == "Dash speed" then
                local t = sliderToTotalTime(value)
                Indicator.Text = string.format("%.2fs", t)
            elseif name == "Dash Degrees" then
                local deg = sliderToDegrees(value)
                Indicator.Text = string.format("%d°", math.floor(deg))
            elseif name == "Dash gap" then
                local r = sliderToOrbitRadius(value)
                Indicator.Text = string.format("%.1f", r)
            else
                Indicator.Text = tostring(value)
            end
        end
        updateIndicator()
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if isRagdolled() then return end
    if (input.UserInputType == Enum.UserInputType.Keyboard and (input.KeyCode == Enum.KeyCode.X or input.KeyCode.DPadUp)) or (input.UserInputType == Enum.UserInputType.MouseButton1 and input.Target and input.Target.Name == "DashButton") then
        -- cooldown removed
        local target = chooseTarget()
        if target then smoothCircle480_then_cameraAim(target) end
    end
...
function smoothCircle480_then_cameraAim(targetModel)
    ...
    local speedPercent = currentSliderValues["Dash speed"]
    if not speedPercent then
        if savedSettings and savedSettings.Sliders and tonumber(savedSettings.Sliders["Dash speed"]) then
            speedPercent = math.clamp(math.floor(tonumber(savedSettings.Sliders["Dash speed"])), 0, 100)
        else
            -- default mapped to ~0.82s -> percent 49
            speedPercent = 49
        end
    end
    local degreesPercent = currentSliderValues["Dash Degrees"]
    if not degreesPercent then
        if savedSettings and savedSettings.Sliders and tonumber(savedSettings.Sliders["Dash Degrees"]) then
            degreesPercent = math.clamp(math.floor(tonumber(savedSettings.Sliders["Dash Degrees"])), 0, 100)
        else
            -- default to 406 degrees -> percent 31
            degreesPercent = 31
        end
    end
    local gapPercent = currentSliderValues["Dash gap"]
    if not gapPercent then
        if savedSettings and savedSettings.Sliders and tonumber(savedSettings.Sliders["Dash gap"]) then
            gapPercent = math.clamp(math.floor(tonumber(savedSettings.Sliders["Dash gap"])), 0, 100)
        else
            -- default to 2.5 studs -> percent 13
            gapPercent = 13
        end
    end
...
print("[Merged] Ready — Settings + Circular Dash merged into one script with Target Select and improved sliders.")
...
-- Timings (user requested: visible for 4 seconds before fade out)
local VISIBLE_TIME = 4.0       -- time fully visible before starting fade out
local FADE_IN_TIME = 0.45
local FADE_OUT_TIME = 0.6

-- Create ScreenGui
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "MerebennieIntro"
screenGui.DisplayOrder = 9999
screenGui.ResetOnSpawn = false

-- Try to protect gui if exploit provides a protect function (common across executors)
...
-- Parent to CoreGui if possible, otherwise fallback to PlayerGui
local ok, err = pcall(function() screenGui.Parent = CoreGui end)
if not ok or not screenGui.Parent then
    local pg = (LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui"))
    if not pg then
        pg = Players.LocalPlayer and Players.LocalPlayer:WaitForChild("PlayerGui")
    end
    screenGui.Parent = pg or Players.LocalPlayer.PlayerGui
end

-- attempt to protect after parenting
pcall(tryProtect, screenGui)

-- Main text label (center)
local label = Instance.new("TextLabel")
label.Name = "MadeByLabel"
label.Parent = screenGui
label.AnchorPoint = Vector2.new(0.5, 0.5)
label.Position = UDim2.new(0.5, 0, 0.5, 0) -- center
label.Size = UDim2.new(0.8, 0, 0.18, 0)      -- wide but not too tall
label.BackgroundTransparency = 1
label.Text = "Made by Merebennie"
label.TextScaled = true
label.RichText = false
label.Font = Enum.Font.LuckiestGuy  -- cool bold font; change if you want
label.TextColor3 = Color3.fromRGB(255, 255, 255)
label.TextTransparency = 1     -- start invisible
label.TextWrapped = true

-- Use TextStroke to outline only the letters (not a box)
label.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
label.TextStrokeTransparency = 1  -- start invisible (we'll tween this)

-- Optional subtle shadow (a blurred duplicate behind the text for depth)
local shadow = Instance.new("TextLabel")
shadow.Name = "Shadow"
shadow.Parent = screenGui
shadow.AnchorPoint = label.AnchorPoint
-- slight downward offset for shadow
...
tween(label, {TextTransparency = 0}, FADE_IN_TIME)
tween(label, {TextStrokeTransparency = 0}, FADE_IN_TIME)  -- reveal outline
tween(shadow, {TextTransparency = 0.45}, FADE_IN_TIME)

-- Wait for fade in to finish then wait the visible time, then fade out
delay(FADE_IN_TIME, function()
    wait(VISIBLE_TIME)
    -- Fade out (text & stroke & shadow)
    tween(label, {TextTransparency = 1}, FADE_OUT_TIME)
    tween(label, {TextStrokeTransparency = 1}, FADE_OUT_TIME)
    tween(shadow, {TextTransparency = 1}, FADE_OUT_TIME)
    -- Destroy after fade out
    delay(FADE_OUT_TIME + 0.05, function()
