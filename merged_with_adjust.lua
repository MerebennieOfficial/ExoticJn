...
local function createExtraFrame()
    ...
    speedMinus.MouseButton1Click:Connect(function()
        ...
        SPEED_SETTING = math.max(0.4, math.floor((SPEED_SETTING - 0.1) * 10 + 0.5) / 10)
        if SPEED_SETTING < 0.4 then SPEED_SETTING = 0.4 end
        speedValue.Text = string.format("%.1f", SPEED_SETTING)
    end)

    speedPlus.MouseButton1Click:Connect(function()
        -- increase SPEED_SETTING by 0.1, clamp to 3.0 maximum
        SPEED_SETTING = math.min(3.0, math.floor((SPEED_SETTING + 0.1) * 10 + 0.5) / 10)
        speedValue.Text = string.format("%.1f", SPEED_SETTING)
    end)
