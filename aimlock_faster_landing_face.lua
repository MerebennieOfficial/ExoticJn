...
--  * Aimlock made more responsive: camera set to Scriptable and updated on RenderStepped for immediate look updates.
--  * When landing, the player will face exactly the target's facing direction (uses target's LookVector).
--  * Keeps prior improvements: velocity-based arc movement, adjustable speed/radius, UI, etc.
-- Note: This file is intended for Delta Executor (mobile compatible tweaks present).
...
local function smoothArcToTarget(targetModel)
    ...
    local oldCamType, oldCamCFrame = nil, nil
    ...
    -- store camera state and switch to Scriptable for instant aim updates
    if cam then
        pcall(function()
    ...
    local targetYaw = math.atan2(lookVec.Z, lookVec.X) -- target's facing yaw
    local startTime = tick()
    local prevTime = startTime
    local conn
    local renderConn

    -- RenderStepped connection to make camera aimlock extra responsive (runs every frame)
    renderConn = RunService.RenderStepped:Connect(function()
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            pcall(function()
                local camPos = cam.CFrame.Position
                -- aim slightly ahead of the target (more natural feel); uses lookVec
                cam.CFrame = CFrame.new(camPos, targetHRP.Position + lookVec * 0.5)
            end)
        end
    end)
    ...
    conn = RunService.Heartbeat:Connect(function(delta)
        if not targetHRP or not targetHRP.Parent then
            ...
            -- restore camera
            if cam and oldCamType then pcall(function() cam.CameraType = oldCamType end) end
            busy = false
            return
        end
        local now = tick()
        ...
        -- Orientation yaw calculation (interpolate toward the target's facing yaw)
        local deltaHRPYaw = shortestAngleDelta(targetYaw, startHRPYaw)
        ...
        -- update rotation only (don't teleport position) to avoid jitter — face interpolated target-facing
        pcall(function()
            if HRP and HRP.Parent then
        ...
        -- Ensure aimlock is kept (RenderStepped handles camera every frame)

        if t >= 1 then
            if conn and conn.Connected then conn:Disconnect() end
            if renderConn and renderConn.Connected then renderConn:Disconnect() end
            -- stop movement cleanly and make sure we face exactly where the target is facing
            pcall(function()
                if HRP and HRP.Parent then
                    HRP.AssemblyLinearVelocity = Vector3.new(0,0,0)
                    -- face exactly the target's lookvector on landing
                    local targetLook = targetHRP.CFrame.LookVector
                    HRP.CFrame = CFrame.new(posNow, posNow + Vector3.new(targetLook.X, 0, targetLook.Z))
                end
            end)
            ...
            if humanoid and oldAutoRotate ~= nil then pcall(function() humanoid.AutoRotate = oldAutoRotate end) end
            -- restore camera type and optionally restore previous camera orientation
