...
local function smoothArcToTarget(targetModel)
    ...
    local conn
    -- store original camera CFrame so we can restore it when aimlock ends
    local camOldCFrame = nil
    if Workspace.CurrentCamera and Workspace.CurrentCamera.CFrame then
        camOldCFrame = Workspace.CurrentCamera.CFrame
    end
    conn = RunService.Heartbeat:Connect(function(delta)
        if not targetHRP or not targetHRP.Parent then
            ...
            pcall(function() if camOldCFrame and Workspace.CurrentCamera then Workspace.CurrentCamera.CFrame = camOldCFrame end end)
        ...
        -- Aimlock: point camera to the target's front or back depending on arc direction.
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            pcall(function()
                local camPos = cam.CFrame.Position
                -- determine whether we're approaching the target from front or back dynamically
                local tgtLook = targetHRP.CFrame.LookVector
                local toMeNow = HRP.Position - targetHRP.Position
                local forwardDotNow = tgtLook:Dot(toMeNow)
                local aimPos
                if forwardDotNow > 0 then
                    -- approaching the back: aim at the back torso position (offset behind)
                    aimPos = targetHRP.Position - tgtLook * BEHIND_DISTANCE
                else
                    -- approaching the front: aim at the front torso position (offset forward)
                    aimPos = targetHRP.Position + tgtLook * FRONT_DISTANCE
                end
        ...
        if t >= 1 then
            ...
            pcall(function()
                if HRP and HRP.Parent then
                    ...
                    HRP.CFrame = CFrame.new(posNow, posNow + Vector3.new(math.cos(currentDesiredHRPYaw), 0, math.sin(currentDesiredHRPYaw)))
                end
            end)
            -- restore camera to previous state (end aimlock)
            pcall(function() if camOldCFrame and cam and cam.CFrame then cam.CFrame = camOldCFrame end end)
