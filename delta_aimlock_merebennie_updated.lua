...
-- Delta Aimlock — Merebennie Edition (Updated UI & Spawn Point)
-- Optimized for Delta Mobile Executor
-- Changes: 70x70 indicator main button, circular Aimbot button (text-only),
-- simplified settings (prediction toggle only + smoothing + aim mode + FOV),
-- low-poly FPS Booster (reduces detail globally), gradients, UI animations,
-- Spawn Point system (set spawn / teleport to spawn), clean & compact settings UI.
...
local function safeParent(obj, parent)
    if obj and parent then pcall(function() obj.Parent = parent end) end
end
...
local CFG = {
    ...
    CLICK_SOUND_ID = "rbxassetid://6042053626", -- button click

    -- Prediction (simple toggle only)
    USE_PREDICTION = true,
...
gui.Name = "DeltaAim_UI_Merebennie"
...
local function tween(instance, props, time, style, dir)
    return TweenService:Create(instance, TweenInfo.new(time or 0.18, style or Enum.EasingStyle.Quad, dir or Enum.EasingDirection.Out), props)
end

-- Watermark (top-left, pixel font)
...
applyTextStyle(watermark, 16, true)
...
addGradient(mainFrame, Color3.fromRGB(25, 40, 65), Color3.fromRGB(40, 80, 120), 90)
...
makeRound(aimbotCircle, 35)
addStroke(aimbotCircle, 1)
addGradient(aimbotCircle, Color3.fromRGB(255,160,80), Color3.fromRGB(255,100,60), 0)
...
-- Settings panel (clean, near & compact)
local settingsFrame = safeNew("Frame")
settingsFrame.Size = UDim2.new(0, 300, 0, 180)
settingsFrame.Position = UDim2.new(0.5, -150, 0.3, 0)
settingsFrame.AnchorPoint = Vector2.new(0.5,0)
settingsFrame.BackgroundTransparency = 0
settingsFrame.Visible = false
safeParent(settingsFrame, gui)
makeRound(settingsFrame, 12); addStroke(settingsFrame, 1); addGradient(settingsFrame, Color3.fromRGB(220,235,255), Color3.fromRGB(200,220,240), 90)

local sTitle = safeNew("TextLabel"); sTitle.Size = UDim2.new(1,0,0,28); sTitle.Position = UDim2.new(0,0,0,6); sTitle.Text = "Settings"; applyTextStyle(sTitle, 16, true); safeParent(sTitle, settingsFrame)

-- Compact clean rows (non-repetitive buttons: toggles use pill buttons)
local function makeRow(container, y)
    local row = safeNew("Frame"); row.Size = UDim2.new(1, -16, 0, 36); row.Position = UDim2.new(0, 8, 0, y); row.BackgroundTransparency = 1; safeParent(row, container)
    return row
end

local function makePillToggle(parent, label, default, onChange, x)
    local pill = safeNew("Frame"); pill.Size = UDim2.new(0.62,0,0,28); pill.Position = UDim2.new(0, 8, 0, x); safeParent(pill, parent)
    pill.BackgroundTransparency = 0; pill.BackgroundColor3 = Color3.fromRGB(30,30,40); makeRound(pill, 14); addStroke(pill, 1)
    local txt = safeNew("TextLabel"); txt.Size = UDim2.new(0.7,0,1,0); txt.Position = UDim2.new(0,8,0,0); txt.Text = label; applyTextStyle(txt, 14, false); txt.TextXAlignment = Enum.TextXAlignment.Left; safeParent(txt, pill)
    local toggle = safeNew("TextButton"); toggle.Size = UDim2.new(0.28, -16, 0, 22); toggle.Position = UDim2.new(0.7, 0, 0.1, 0); toggle.Text = default and "ON" or "OFF"; applyTextStyle(toggle, 14, true); safeParent(toggle, pill); makeRound(toggle, 10); addStroke(toggle, 1); addGradient(toggle, Color3.fromRGB(120,220,120), Color3.fromRGB(40,180,80), 0)
    toggle.MouseButton1Click:Connect(function()
        if clickSound then clickSound:Play() end
        local v = toggle.Text == "ON"
        v = not v
...
local function makeSliderRow(parent, label, min, max, step, default, y)
    local row = makeRow(parent, y)
    local txt = safeNew("TextLabel"); txt.Size = UDim2.new(0.6,0,1,0); txt.Position = UDim2.new(0,0,0,0); txt.Text = label .. ": " .. string.format("%.2f", default); applyTextStyle(txt, 14, false); safeParent(txt, row)
    local minus = safeNew("TextButton"); minus.Size = UDim2.new(0, 26, 0, 24); minus.Position = UDim2.new(0.62, 8, 0, 2); minus.Text = "-"; applyTextStyle(minus, 16, true); safeParent(minus, row); makeRound(minus, 6); addStroke(minus, 1)
    local plus = safeNew("TextButton"); plus.Size = UDim2.new(0, 26, 0, 24); plus.Position = UDim2.new(0.86, 8, 0, 2); plus.Text = "+"; applyTextStyle(plus, 16, true); safeParent(plus, row); makeRound(plus, 6); addStroke(plus, 1)
    local value = default
    ...
    minus.MouseButton1Click:Connect(function() if settingsFrame.Visible then update(update(value) - step); if clickSound then clickSound:Play() end end)
    plus.MouseButton1Click:Connect(function() if settingsFrame.Visible then update(update(value) + step); if clickSound then clickSound:Play() end end)
    update(default)
    return row
end

-- Settings content (clean & minimal)
makePillToggle(settingsFrame, "Prediction (On/Off)", CFG.USE_PREDICTION, function(v) CFG.USE_PREDICTION = v end, 40)
makeSliderRow(settingsFrame, "Aim Smoothing", 0, 1, 0.05, CFG.SMOOTHING, 80)
do -- FOV row
    local row = makeRow(settingsFrame, 120)
    local txt = safeNew("TextLabel"); txt.Size = UDim2.new(0.6,0,1,0); txt.Position = UDim2.new(0,0,0,0); txt.Text = "FOV Size: " .. tostring(CFG.FOV_PIXELS); applyTextStyle(txt, 14, false); safeParent(txt, row)
    local less = safeNew("TextButton"); less.Size = UDim2.new(0, 26, 0, 24); less.Position = UDim2.new(0.62, 8, 0, 2); less.Text = "-"; applyTextStyle(less, 16, true); safeParent(less, row); makeRound(less, 6); addStroke(less, 1)
    local more = safeNew("TextButton"); more.Size = UDim2.new(0, 26, 0, 24); more.Position = UDim2.new(0.86, 8, 0, 2); more.Text = "+"; applyTextStyle(more, 16, true); safeParent(more, row); makeRound(more, 6); addStroke(more, 1)
    less.MouseButton1Click:Connect(function() if not settingsFrame.Visible then return end; CFG.FOV_PIXELS = math.max(20, CFG.FOV_PIXELS - 10); fovFrame.Size = UDim2.new(0, CFG.FOV_PIXELS*2, 0, CFG.FOV_PIXELS*2); if clickSound then clickSound:Play() end end)
    more.MouseButton1Click:Connect(function() if not settingsFrame.Visible then return end; CFG.FOV_PIXELS = math.min(400, CFG.FOV_PIXELS + 10); fovFrame.Size = UDim2.new(0, CFG.FOV_PIXELS*2, 0, CFG.FOV_PIXELS*2); if clickSound then clickSound:Play() end end)
end

-- Spawn point rows
local spawnSetBtn = safeNew("TextButton"); spawnSetBtn.Size = UDim2.new(0.46, -12, 0, 30); spawnSetBtn.Position = UDim2.new(0.02, 8, 0, 152); spawnSetBtn.Text = "Set Spawn"; applyTextStyle(spawnSetBtn, 14, true); safeParent(spawnSetBtn, settingsFrame); makeRound(spawnSetBtn, 8); addStroke(spawnSetBtn, 1); addGradient(spawnSetBtn, Color3.fromRGB(255,120,120), Color3.fromRGB(200,80,80), 0)
local spawnTPBtn  = safeNew("TextButton"); spawnTPBtn.Size = UDim2.new(0.46, -12, 0, 30); spawnTPBtn.Position = UDim2.new(0.5, 8, 0, 152); spawnTPBtn.Text = "Teleport to Spawn"; applyTextStyle(spawnTPBtn, 14, true); safeParent(spawnTPBtn, settingsFrame); makeRound(spawnTPBtn, 8); addStroke(spawnTPBtn, 1); addGradient(spawnTPBtn, Color3.fromRGB(120,200,255), Color3.fromRGB(60,140,220), 0)

-- Settings open/close animation
settingsBtn.MouseButton1Click:Connect(function()
    if clickSound then clickSound:Play() end
    if settingsFrame.Visible then
        tween(settingsFrame, {Position = UDim2.new(0.5, -150, 0.2, -40), Size = UDim2.new(0,300,0,180)}, 0.12):Play()
        tween(settingsFrame, {Position = UDim2.new(0.5, -150, 0.5, 0), Size = UDim2.new(0,0,0,0)}, 0.18):Play()
        ...
        tween(settingsFrame, {Position = UDim2.new(0.5, -150, 0.3, 0), Size = UDim2.new(0,300,0,180)}, 0.22):Play()
    end
end)

-- Toggle aim via aimbotCircle or a key (V)
aimbotCircle.MouseButton1Click:Connect(function()
    aiming = not aiming
    targetPart = nil; targetHRP = nil
    if clickSound then clickSound:Play() end
...
-- FPS booster: lowpoly everything (reduce particles, disable postfx, lower materials & shadows)
local LowPolyBooster = {}
LowPolyBooster.enabled = false
LowPolyBooster.cache = {postfx = {}, particles = {}, trails = {}, beams = {}, materials = {}, shadows = nil, castshadows = {}}

function LowPolyBooster:scan()
    self.cache.postfx = {}
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("DepthOfFieldEffect") or child:IsA("BloomEffect") or child:IsA("BlurEffect") or child:IsA("SunRaysEffect") or child:IsA("ColorCorrectionEffect") then
            table.insert(self.cache.postfx, {obj = child, enabled = child.Enabled})
        end
    end
...
LowPolyBooster:setEnabled = function(self, v)
    if v then self:apply() else self:restore() end
end

-- Hook the booster toggle button created earlier
-- We'll add an inline toggle beside FPS label (small pill)
local boosterToggle = safeNew("TextButton")
boosterToggle.Size = UDim2.new(0, 60, 0, 24)
boosterToggle.Position = UDim2.new(1, -80, 0, 10)
...
-- Targeting helpers (same logic, but prediction toggle only)
...
local function computeLeadPosition(part, hrp)
    ...
    local ping = getPingSeconds() -- seconds
...
local function pickBest(ignoreFOV)
    ...
    targetPart = bestPart; targetHRP = bestHRP
    return targetPart
end
...
local function aimAt(pos)
    ...
    if CFG.AIM_MODE == "Snap" or CFG.SMOOTHING <= 0 then
        ...
        local cur = Camera.CFrame; local alpha = math.clamp(CFG.SMOOTHING, 0, 1)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- Camera-based switching (simplified: instant pick on look movement)
    local look = Camera.CFrame.LookVector
    local yaw = math.atan2(look.X, look.Z); local pitch = math.asin(-look.Y); local now = tick()
    if lastYaw ~= nil then
        local dy = math.abs(yaw - lastYaw) if dy > math.pi then dy = math.abs(dy - 2*math.pi) end
        local dp = math.abs(pitch - lastPitch)
        if (dy >= 0.006 or dp >= 0.02) and (now - lastSwitchTick) >= 0.08 then
            pickBest(true)
            lastSwitchTick = now
        end
    end
    ...
    if aiming then
        ...
        if targetPart and targetHRP then
            ...
            if within then
                if not CFG.REQUIRE_VISIBLE then keep = true else
                    ...
                    if seen then keep = true; lastSeenVisibleAt = now else keep = (now - lastSeenVisibleAt) <= CFG.VIS_STICKY_TIME end
                end
            end
        end
...
-- GUI resilience
task.spawn(function() while task.wait(2) do if not gui.Parent then safeParent(gui, PlayerGui) end end end)
...
local function teleportToSpawnPoint()
    ...
    local hrp = char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso")
    if not hrp then return end
    -- Move character above spawn point
...
-- Settings: smoothing slider handler (live update)
-- (slider row already updates CFG.SMOOTHING via UI interactions above)

-- Open/close animations already implemented for settings

-- Auto-enable booster default
LowPolyBooster:setEnabled(CFG.BOOSTER_DEFAULT)

print("[DeltaAim] Updated UI + SpawnPoint build loaded.") 
