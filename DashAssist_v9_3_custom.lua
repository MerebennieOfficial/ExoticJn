...
--// Merebennie DashAssist v9.3 - Custom UI (Yaw-only aimlock preserving camera pitch)
--// Modified to match requested UI layout & ensure aimlock does NOT change camera pitch (only yaws left/right).
...
-- Aimlock utility: while enabled, force the camera to yaw toward target but preserve camera pitch (Y component of look vector)
...
local function startAimlockFor(targetCFrame)
    ...
    aimlockConnection = RunService.Heartbeat:Connect(function()
        ...
        if cam and cam.CFrame and aimlockTargetPos then
            ...
            local dx = aimlockTargetPos.X - camPos.X
            local dz = aimlockTargetPos.Z - camPos.Z
            -- preserve current camera pitch (vertical look component)
            local preservedY = cam.CFrame.LookVector.Y
            local lookVec = Vector3.new(dx, preservedY, dz)
            if lookVec.Magnitude == 0 then return end
            pcall(function() cam.CFrame = CFrame.new(camPos, camPos + lookVec) end)
        end
    end)
end
...
local function smoothArcToBack(targetModel)
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        if cam and cam.CFrame then
            -- Preserve camera pitch while yawing toward target center
            local camPos = cam.CFrame.Position
...
-- Compact Settings UI (customized to match provided design)
local function createSettingsUI()
    ...
    mainFrame.Size = UDim2.new(0,360,0,420) -- increased to match requested visual
    ...
    -- Minimize button (styled like small rounded square top-right)
    local minimizeBtn = Instance.new("TextButton")
    minimizeBtn.Name = "Minimize"
    minimizeBtn.Size = UDim2.new(0,38,0,28)
    minimizeBtn.Position = UDim2.new(1,-50,0,10)
    minimizeBtn.Text = "-"
    minimizeBtn.Font = Enum.Font.Gotham
    minimizeBtn.TextSize = 18
    minimizeBtn.BackgroundColor3 = Color3.fromRGB(120,120,125)
    ...
    if savedX and savedY then
        ...
        miniButton.Position = UDim2.new(0.03,0,0.12,0)
    end
    miniButton.Text = ">"
    ...
    -- Player label and scrolling list
    local playersLabel = Instance.new("TextLabel")
    playersLabel.Name = "PlayersLabel"
    playersLabel.Size = UDim2.new(1,-24,0,20)
    playersLabel.Position = UDim2.new(0,14,0,62)
    ...
    scroll.Size = UDim2.new(1,-28,0,160)
    ...
    -- Find nearest and M1 row
    local bottomRow = Instance.new("Frame")
    bottomRow.Name = "BottomRow"
    bottomRow.Size = UDim2.new(1,-28,0,52)
    bottomRow.Position = UDim2.new(0,14,0,260)
    ...
    findNearestBtn.Size = UDim2.new(0.48,0,1,0)
    findNearestBtn.Position = UDim2.new(0,0,0,0)
    findNearestBtn.BackgroundColor3 = Color3.fromRGB(36,36,40)
    findNearestBtn.Parent = bottomRow
    local fnCorner = Instance.new("UICorner", findNearestBtn)
    fnCorner.CornerRadius = UDim.new(0,12)
    ...
    fnDot.Name = "Dot"
    fnDot.Size = UDim2.new(0,16,0,16)
    fnDot.Position = UDim2.new(1,-24,0.5,-8)
    ...
    m1Btn.Name = "M1After"
    m1Btn.Size = UDim2.new(0.22,0,1,0)
    m1Btn.Position = UDim2.new(0.51,12,0,0)
    m1Btn.BackgroundColor3 = Color3.fromRGB(36,36,40)
    m1Btn.Parent = bottomRow
    local m1Corner = Instance.new("UICorner", m1Btn)
    m1Corner.CornerRadius = UDim.new(0,12)
    ...
    m1Dot.Name = "Dot"
    m1Dot.Size = UDim2.new(0,16,0,16)
    m1Dot.Position = UDim2.new(1,-24,0.5,-8)
    m1Dot.BackgroundTransparency = 1
    m1Dot.Parent = m1Btn
    local m1DotCorner = Instance.new("UICorner", m1Dot)
    m1DotCorner.CornerRadius = UDim.new(0,8)

    -- Delay label + textbox (textbox aligned next to label)
    local delayLabel = Instance.new("TextLabel")
    delayLabel.Name = "DelayLabel"
    delayLabel.Size = UDim2.new(0.36,0,0,20)
    delayLabel.Position = UDim2.new(0,14,0,330)
    ...
    delayBox.Size = UDim2.new(0,86,0,28)
    delayBox.Position = UDim2.new(0.34,14,0,326) -- beside label
    ...
    -- ESP toggle aligned to right
    local espBtn = Instance.new("Frame")
    espBtn.Name = "EspToggle"
    espBtn.Size = UDim2.new(0.28,0,0,36)
    espBtn.Position = UDim2.new(0.74,8,0,326)
    ...
    espDot.Name = "Dot"
    espDot.Size = UDim2.new(0,16,0,16)
    espDot.Position = UDim2.new(1,-24,0.5,-8)
    ...
    local function refreshPlayerList()
        ...
        local total = 0
        for _, c in pairs(scroll:GetChildren()) do
            if c:IsA("TextButton") then total = total + c.Size.Y.Offset + 10 end
        end
...
print("[DashAssist v9.3 - Custom] Ready. Compact settings UI loaded. Activation requires a target within " .. ACTIVATION_RANGE .. " studs.")
