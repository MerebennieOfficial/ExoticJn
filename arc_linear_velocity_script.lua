...
local TOTAL_TIME = 0.35
local SPEED_SETTING = 0.9
...
local function smoothArcToTarget(targetModel)
    ...
    local targetFeetY = center.Y
    local desiredLandingY = targetFeetY + 0.6
    finalPos = Vector3.new(finalPos.X, desiredLandingY, finalPos.Z)

    local startPos = HRP.Position
    local startY = startPos.Y
    if startY - targetFeetY > 1.5 then
        startY = targetFeetY + 1.0
    end

    local startRadius = (Vector3.new(startPos.X,0,startPos.Z) - Vector3.new(center.X,0,center.Z)).Magnitude
    ...
    local startAngle = math.atan2(startPos.Z - center.Z, startPos.X - center.X)
    ...
    local function ensureLinearVelocity(part)
        ...
        local att = part:FindFirstChild("MerebennieDash_Attachment")
        if not att then
            att = Instance.new("Attachment")
            att.Name = "MerebennieDash_Attachment"
            att.Parent = part
        end
        local lv = part:FindFirstChild("Merebennie_LinearVelocity")
        if not lv then
            lv = Instance.new("LinearVelocity")
            lv.Name = "Merebennie_LinearVelocity"
            lv.Attachment0 = att
    ...
    local lv = ensureLinearVelocity(HRP)
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        if not targetHRP or not targetHRP.Parent then
            ...
            pcall(function() if lv then lv.VectorVelocity = Vector3.new(0,0,0) end end)
            busy = false
            return
        end
        local now = tick()
        ...
        local maxLift = 1.0
        local startYClamped = math.max(startY, targetFeetY)
        local desiredY = startYClamped + (desiredLandingY - startYClamped) * e
        desiredY = math.clamp(desiredY, targetFeetY - 0.2, targetFeetY + maxLift)

        local posNow = Vector3.new(x, desiredY, z)

        local currentPos = HRP.Position
        local deltaPos = posNow - currentPos
        local desiredVel
        if dt and dt > 0 then
            desiredVel = deltaPos / dt
        else
            desiredVel = deltaPos * 60
        end

        local maxVerticalSpeed = 60
        desiredVel = Vector3.new(desiredVel.X, math.clamp(desiredVel.Y, -maxVerticalSpeed, maxVerticalSpeed), desiredVel.Z)

        pcall(function()
            if lv then
                lv.VectorVelocity = desiredVel
            end
        end)
        ...
        local e_rot = math.clamp(e * 2.2, 0, 1)
        local hrpYawNow = startHRPYaw + deltaHRPYaw * e_rot
        local hrpLook = Vector3.new(math.cos(hrpYawNow), 0, math.sin(hrpYawNow))

        pcall(function()
            local posForCFrame = HRP.Position
            HRP.CFrame = CFrame.new(posForCFrame, posForCFrame + hrpLook)
        end)
        ...
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            ...
            local e_cam = math.clamp(e * 2.0, 0, 1)
        ...
        if t >= 1 then
            ...
            pcall(function() if lv then lv.VectorVelocity = Vector3.new(0,0,0) end end)
            pcall(function()
                HRP.CFrame = CFrame.new(finalPos, finalPos + Vector3.new(math.cos(currentDesiredHRPYaw or 0), 0, math.sin(currentDesiredHRPYaw or 0)))
            end)
