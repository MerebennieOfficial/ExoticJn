...
local function getNearestTarget()
    -- Optimized: iterate Players instead of workspace:GetDescendants for performance
    local nearest, dist = nil, 20
    if not HumanoidRootPart then return nil end
    for _, pl in pairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer and pl.Character then
            local v = pl.Character
            local vhrp = v:FindFirstChild("HumanoidRootPart")
            if vhrp then
                local ok, mag = pcall(function()
                    return (HumanoidRootPart.Position - vhrp.Position).Magnitude
                end)
...
-- Replace per-frame zeroing with a short one-time velocity reset to reduce CPU usage
antiConn = nil
task.spawn(function()
...
-- Use TweenService to move the HumanoidRootPart to the target pose smoothly without setting CFrame each frame.
if HumanoidRootPart then
    local targetCFrame = nil
    local okTarget, tcf = pcall(function()
        return CFrame.new(targetHRP.Position - targetHRP.CFrame.LookVector * 0.3) * CFrame.Angles(math.rad(60), 0, 0)
    end)
    if okTarget and tcf then
        targetCFrame = tcf
    end

    if targetCFrame then
        local tweenInfo = TweenInfo.new(layDuration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        ...
        if okTween and tween then
            ...
            pcall(function() HumanoidRootPart.CFrame = targetCFrame end)
