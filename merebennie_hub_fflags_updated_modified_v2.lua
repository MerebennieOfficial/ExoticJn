...
-- Sensitivity slider + bypass mobile cap
local sensitivityMultiplier = 1.0
local bypassMobileCapEnabled = false
Tab:CreateSlider({
    Name = "Sensitivity",
    Range = {0.1, 5},
    Increment = 0.1,
...
-- Toggle: Bypass Mobile Cap (enables custom camera control using the slider)
Tab:CreateToggle({
    Name = "Bypass Mobile Cap (Custom camera control)",
    CurrentValue = false,
    Flag = "BypassMobileCap",
    Callback = function(state)
        bypassMobileCapEnabled = state
        local Camera = workspace.CurrentCamera
        if state then
            -- store previous camera type to restore later
            local prevType = Camera.CameraType
            Camera:SetAttribute("PrevCameraType", tostring(prevType))
            Camera.CameraType = Enum.CameraType.Scriptable
            -- start custom camera control
            ...
            conn = UIS.InputChanged:Connect(function(input)
                if not bypassMobileCapEnabled then
                ...
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement then
                    if lastPos then
                        ...
                        -- apply sensitivity multiplier; invert Y for natural feel
                        rotX = rotX - delta.X * 0.002 * sensitivityMultiplier
                        rotY = math.clamp(rotY - delta.Y * 0.002 * sensitivityMultiplier, -math.pi/2, math.pi/2)
                    end
                    lastPos = input.Position
                end
            end)

            -- also clear lastPos on touch end / mouse end
            ...
            cameraConn = RunService.RenderStepped:Connect(function()
                ...
                -- build a rotation from rotX, rotY
                local rotation = CFrame.Angles(rotY, rotX, 0)
                Camera.CFrame = CFrame.new(basePos) * rotation
            end)

            -- store connections so we can disconnect when disabling
            Camera:SetAttribute("CustomCameraConn", true)
        else
            -- disable custom camera control
            local Camera = workspace.CurrentCamera
            -- restore previous camera type
            local prevTypeStr = Camera:GetAttribute("PrevCameraType")
            if prevTypeStr then
                local ok, enumVal = pcall(function() return Enum.CameraType[prevTypeStr] end)
                if ok and enumVal then
                    Camera.CameraType = enumVal
                end
            end
            -- Disconnecting is handled by checking bypassMobileCapEnabled inside handlers
        end
    end
})
