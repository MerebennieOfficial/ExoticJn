...
-- Hey.lua
-- Merged circular dash -> converted to LinearVelocity
-- Adjusted defaults per request (ARC_APPROACH_RADIUS 9, SPEED_SETTING slower, BEHIND/FRONT = 3, MAX_RANGE 30)
-- Uses LinearVelocity (Attachment + Vector mode) for movement to avoid BodyVelocity/AssemblyVelocity.
-- Designed as a LocalScript and kept Delta Executor compatibility in mind (uses PlayerGui, LocalPlayer etc).
...
local MAX_RANGE = 30 -- changed from 40 to 30
local ARC_APPROACH_RADIUS = 9 -- changed from 11 to 9
local BEHIND_DISTANCE = 3 -- on-land stud gap changed from 4 to 3
local FRONT_DISTANCE = 3   -- same as above
local TOTAL_TIME = 0.3
local SPEED_SETTING = 0.8 -- made default a bit slower
...
-- NEW: LinearVelocity-based arc movement
local function ensureRootAttachment(part)
    if not part then return nil end
    local att = part:FindFirstChild("DashRootAttachment")
    if att and att:IsA("Attachment") then return att end

    -- Reuse RootAttachment if present, but create our own to control position
    local rootAtt = part:FindFirstChild("RootAttachment")
    if rootAtt and rootAtt:IsA("Attachment") then
        rootAtt.Name = "DashRootAttachment"
        return rootAtt
    end
    att = Instance.new("Attachment")
    att.Name = "DashRootAttachment"
    att.Parent = part
    return att
end

local function createLinearVelocity(parentPart)
    if not parentPart then return nil end
    local lv = Instance.new("LinearVelocity")
    lv.Name = "DashLinearVelocity"
    lv.Attachment0 = ensureRootAttachment(parentPart)
    -- Position the attachment at the assembly center of mass to avoid floating and weird rotations
    pcall(function()
        if lv.Attachment0 and parentPart.AssemblyCenterOfMass then
            lv.Attachment0.WorldPosition = parentPart.AssemblyCenterOfMass
        end
    end)
...
local function destroyLinearVelocityIfExists(part)
    if not part then return end
    local existing = part:FindFirstChild("DashLinearVelocity")
...
local function smoothArcToTarget(targetModel)
    ...
    destroyLinearVelocityIfExists(HRP)
    local lv = createLinearVelocity(HRP)
    ...
    conn = RunService.Heartbeat:Connect(function(step)
        ...
        -- compute desired orientation for HRP (yaw only)
        ...
        -- Set LinearVelocity target velocity to move HRP toward posNow this frame
        if lv and HRP and HRP.Parent then
            local currentPos = HRP.Position
            -- desired velocity = (posNow - currentPos) / dt
            local dt = step or 1/60
            local desiredVel = (posNow - currentPos) / math.max(0.0001, dt)
            -- clamp vertical component to avoid lifting or floating artifacts (match gravity when possible)
            -- We allow the Y component but keep it reasonable to prevent hovering.
            desiredVel = Vector3.new(desiredVel.X, math.clamp(desiredVel.Y, -50, 50), desiredVel.Z)
            pcall(function() lv.VectorVelocity = desiredVel end)
        end

        -- rotate visual HRP orientation by setting CFrame without moving position (use LookVector yaw)
        if HRP and HRP.Parent then
            pcall(function() HRP.CFrame = CFrame.new(HRP.Position, HRP.Position + hrpLook) end)
        end

        -- camera aimlock: make camera interpolation much faster/accurate (user requested very fast aimlock)
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            ...
            -- speed multiplier for camera interpolation (fast)
            local camSpeedMult = 3.0
            local eCam = math.clamp(e * camSpeedMult, 0, 1)
            ...
            -- apply camera quickly using eCam
...
-- (rest of the original GUI code left mostly unchanged, with a few parameter tweaks below)
        ...
        if not isDragging and not busy then
            ...
            if targetModel then
                ...
                if ok and started then
                    pcall(function() pressSFX:Play() end)
                    -- delay before M1 activates changed from 0.1 to 0.15
                    if m1Enabled then
                        delay(0.15, function() pcall(sendM1) end)
                    end
