    ...
    for i, name in ipairs(TitleLabels) do
        ...
        if currentSliderValues[name] ~= nil then
            ...
            if name == "Dash speed" then
                initValue = 92 -- start towards the faster side (user wanted more than previous max)
            elseif name == "Dash Degrees" then
                -- default to ~480 degrees
                initValue = math.floor((480 - 90) / (1080 - 90) * 100)
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if isRagdolled() then return end
    if (input.UserInputType == Enum.UserInputType.Keyboard and (input.KeyCode == Enum.KeyCode.X or input.KeyCode.DPadUp)) or (input.UserInputType == Enum.UserInputType.MouseButton1 and input.Target and input.Target.Name == "DashButton") 
        -- cooldown removed
        local target = chooseTarget()
        if target then smoothCircle480_then_cameraAim(target) end
    end
