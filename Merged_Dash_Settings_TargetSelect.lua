...
-- Merged: Circular Dash + Settings V2 (with Target-Select support)
-- Behavior changes:
--  * If you select a player from the player list the dash (button and X key) will target that specific player (if within MAX_RANGE = 40).
--  * If no player is selected or the selected player is invalid/out of range, it falls back to "target nearest" (within MAX_RANGE).
--  * Clicking a selected player's button again will deselect them (returning to target-nearest).
--  * If the selected player leaves the game, selection is cleared.
-- Place this as a LocalScript (StarterPlayerScripts or similar).
...
Players.PlayerRemoving:Connect(function(pl)
    ...
    -- refresh UI so list stays accurate
    refreshPlayers()
end)
...
print("[Merged] Ready — Settings + Circular Dash merged into one script with Target Select.")
