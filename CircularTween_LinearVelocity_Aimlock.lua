--[ Modified Circular Tween -> Linear Velocity version
-- Uses AssemblyLinearVelocity to move HRP along an arc path (linear velocity, not BodyVelocity)
-- Aimlock replaced with provided Tween code (0.1s linear CFrame tween to look at target)
-- Defaults changed: ARC_APPROACH_RADIUS = 9, SPEED_SETTING = 0.8
-- BEHIND_DISTANCE & FRONT_DISTANCE changed from 4 to 3
-- Keeps character Y level to avoid floating (uses player's starting HRP Y while arcing)
-- Written for executor usage (mobile + keyboard + controller supported)
-- NOTE: This implementation sets HumanoidRootPart.AssemblyLinearVelocity each frame.
-- If you meant the new Constraint object LinearVelocity, tell me and I can convert it.
...
local MAX_RANGE = 40
local ARC_APPROACH_RADIUS = 9      -- default changed from 11 -> 9
local BEHIND_DISTANCE = 3         -- changed from 4 -> 3
local FRONT_DISTANCE = 3          -- changed from 4 -> 3
local TOTAL_TIME = 0.3
local SPEED_SETTING = 0.8         -- default speed changed from 1.0 -> 0.8
...
-- smoothArcToTarget now uses AssemblyLinearVelocity to move the HRP along the arc.
local function smoothArcToTarget(targetModel)
    ...
    local myPos = HRP.Position
    local myY = myPos.Y -- keep Y constant to prevent floating above ground
    local lookVec = targetHRP.CFrame.LookVector
    local toMe = myPos - center
    local forwardDot = lookVec:Dot(toMe)
    local finalPosLocal
    if forwardDot > 0 then
        finalPosLocal = center - lookVec * BEHIND_DISTANCE
    else
        finalPosLocal = center + lookVec * FRONT_DISTANCE
    end
    local finalPos = Vector3.new(finalPosLocal.X, myY, finalPosLocal.Z) -- use player's Y to avoid floating
    ...
    -- AIMLOCK REPLACEMENT: orient HRP to look at target using provided Tween snippet
    pcall(function()
    ...
    if cam and cam.CFrame then
        local startCamLook = cam.CFrame.LookVector or Vector3.new(0,0,1)
        startPitch = math.asin(math.clamp(startCamLook.Y, -0.999, 0.999))
    end
    local humanoid = nil
    local oldAutoRotate = nil
    local aimlockActive = false
    local oldAutoRotateSaved = nil
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        local y = myY -- keep original Y
        local posNow = Vector3.new(x, y, z)

        -- Compute desired linear velocity to reach posNow this frame
        local curPos = HRP.Position
        local deltaTime = math.max(0.001, now - lastTick)
        lastTick = now
        local desiredVel = (posNow - curPos) / deltaTime

        -- Apply linear velocity (AssemblyLinearVelocity)
        pcall(function()
            HRP.AssemblyLinearVelocity = desiredVel
        end)

        -- Aimlock: activates while HRP has linear velocity above threshold; locks character yaw and camera to target.
        local aimThreshold = 0.5 -- tune sensitivity
        local currentHRP = HRP or hrp
        local rv = Vector3.new(0,0,0)
        pcall(function()
            if currentHRP and currentHRP.AssemblyLinearVelocity then
                rv = currentHRP.AssemblyLinearVelocity
            elseif currentHRP and currentHRP.Velocity then
                rv = currentHRP.Velocity
            end
        end)
        local velMag = rv.Magnitude

        if velMag > aimThreshold then
            -- start aimlock
            if not aimlockActive then
                aimlockActive = true
                if humanoid then
                    pcall(function()
                        if oldAutoRotateSaved == nil then oldAutoRotateSaved = humanoid.AutoRotate end
                        humanoid.AutoRotate = false
                    end)
                end
            end

            -- lock character yaw (keep same Y)
            if targetHRP and targetHRP.Parent and currentHRP then
                local targetLook = Vector3.new(targetHRP.Position.X, currentHRP.Position.Y, targetHRP.Position.Z)
                pcall(function()
                    currentHRP.CFrame = CFrame.new(currentHRP.Position, targetLook)
                end)
            end

            -- lock camera to look at the target (full 3D)
            if cam and targetHRP and targetHRP.Parent then
            ...
            if aimlockActive then
                aimlockActive = false
                if humanoid and oldAutoRotateSaved ~= nil then
                    pcall(function() humanoid.AutoRotate = oldAutoRotateSaved end)
                end
            end

            -- fallback to previous smoothing behavior (preserve original cam smoothing)
        ...
        if t >= 1 then
            if conn and conn.Connected then conn:Disconnect() end
            -- stop motion and snap to final position & orientation facing target
...
-- (The rest of the script UI and controls are mostly kept the same, only defaults changed where mentioned)
...
local function createExtraFrame()
    ...
    speedMinus.MouseButton1Click:Connect(function()
        -- decrease SPEED_SETTING by 0.1, clamp to 0.1 minimum
    ...
    speedPlus.MouseButton1Click:Connect(function()
        -- increase SPEED_SETTING by 0.1, clamp to 5.0 maximum
...
print("[Merged] Circular tween (LinearVelocity) + Settings updated and ready.")
