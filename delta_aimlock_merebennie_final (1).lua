-- Delta Aimlock — Merebennie Final Build (Merged UI + Logic)
-- Designed for Delta Mobile Executor compatibility
-- Features:
-- * 70x70 indicator main button (indicator-only)
-- * Circular "AIMBOT" button (text-only)
-- * Clean, compact settings UI with gradients and animations
-- * Prediction toggle (simple on/off)
-- * Smoothing and Aim Mode options (limited, safe)
-- * FOV ring, FPS counter, Low-Poly FPS Booster (disable heavy effects/particles)
-- * Spawn Point system (Set Spawn / Teleport to Spawn)
-- * LOS visibility check and adaptive prediction (internal, not exposed)
-- * UI parenting to CoreGui for executor compatibility and re-parenting resilience
-- * All UI elements created after PlayerGui/CoreGui are ready
...
-- ======= GUI CREATION (parent to CoreGui for Delta) =======
...
-- Try to parent to CoreGui first (Delta-friendly), fallback to PlayerGui
...
-- ======= GUI RESILIENCE =======
task.spawn(function() while task.wait(2) do if not gui.Parent then pcall(function() gui.Parent = CoreGui end); if not gui.Parent then gui.Parent = PlayerGui end end end end)
