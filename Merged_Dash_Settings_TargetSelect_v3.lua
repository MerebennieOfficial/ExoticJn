-- Merged: Circular Dash + Settings V2 (Target-Select + improved sliders UI & behavior)
-- Updated: Added "stop at 430°" smooth-stop behavior and strict Y-level locking when stopping.
-- Place as a LocalScript (StarterPlayerScripts or similar).
...
-- Aimlock trigger degrees (unchanged) -- stop behavior handled separately at 430deg
...
-- Stop-at-degrees configuration (user requested strict stop at 430 degrees)
local STOP_AT_DEGREES = 430
...
function smoothCircle480_then_cameraAim(targetModel)
    ...
    local speedPercent = currentSliderValues["Dash speed"] or (savedSettings and savedSettings.Sliders and tonumber(savedSettings.Sliders["Dash speed"]) or 92)
    local degreesPercent = currentSliderValues["Dash Degrees"] or (savedSettings and savedSettings.Sliders and tonumber(savedSettings.Sliders["Dash Degrees"]) or math.floor((480-90)/(1080-90)*100))
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        -- Check aimlock trigger like original
        ...
        -- Check for user-requested hard-stop at STOP_AT_DEGREES (ensure smooth stop & lock to target's Y)
        if not stopTriggered then
            local travelledDeg = math.deg(totalAngle * easeOutCubic(t))
            if travelledDeg >= STOP_AT_DEGREES then
                stopTriggered = true
                -- compute the eased fraction corresponding to STOP_AT_DEGREES
                local totalDegVal = totalDeg > 0 and totalDeg or 480
                local y = math.clamp(STOP_AT_DEGREES / totalDegVal, 0, 1)
                -- inverse of easeOutCubic: t = 1 - (1 - y)^(1/3)
                local t_stop = 1 - (1 - y)^(1/3)
                -- compute final angle for strict STOP_AT_DEGREES
                local finalAngle = startAngle + dir * math.rad(STOP_AT_DEGREES)
                local finalRadius = orbitRadius
                local finalPos = Vector3.new(center.X + finalRadius * math.cos(finalAngle), centerY, center.Z + finalRadius * math.sin(finalAngle))
                -- capture current transform
                local curCFrame = HRP.CFrame
                local curPos = curCFrame.Position
                local curLookDir = curCFrame.LookVector
                local curYaw = math.atan2(curLookDir.Z, curLookDir.X)

                -- duration for smooth stop (shorter than full spin; feels responsive but smooth)
                local stopDuration = math.clamp(TOTAL_TIME * 0.25, 0.06, 0.45)
                local stopStart = tick()
                -- Interpolate position & yaw to finalPos over stopDuration using easeOutCubic-like curve
                while tick() - stopStart < stopDuration do
                    if not targetHRP or not targetHRP.Parent or not HRP or not HRP.Parent then break end
                    local sNow = (tick() - stopStart) / stopDuration
                    local se = easeOutCubic(sNow)
                    local interpPos = curPos:Lerp(finalPos, se)
                    -- ensure Y locked to target
                    interpPos = Vector3.new(interpPos.X, centerY, interpPos.Z)
                    -- interpolate yaw safely via shortestAngleDelta
                    local desiredYaw = math.atan2((targetHRP.Position - finalPos).Z, (targetHRP.Position - finalPos).X)
                    local dy = shortestAngleDelta(desiredYaw, curYaw)
                    local interpYaw = curYaw + dy * se
                    pcall(function()
                        HRP.CFrame = CFrame.new(interpPos, interpPos + Vector3.new(math.cos(interpYaw), 0, math.sin(interpYaw)))
                    end)
                    RunService.Heartbeat:Wait()
                end
                -- final snap to ensure exact alignment (same Y as target and facing target)
                if targetHRP and targetHRP.Parent and HRP and HRP.Parent then
                    local finalLookDir = (targetHRP.Position - finalPos)
                    if finalLookDir.Magnitude < 0.001 then finalLookDir = Vector3.new(1,0,0) end
                    pcall(function()
                        HRP.CFrame = CFrame.new(finalPos, finalPos + Vector3.new(finalLookDir.X, 0, finalLookDir.Z).Unit)
                    end)
                end

                -- stop animations and finalize aimlock like normal end
                pcall(function()
                ...
                -- disconnect main conn to end regular spinning updates
                if conn then
...
print("[Merged] Ready — Settings + Circular Dash merged into one script with Target Select and improved sliders. STOP-AT-430° behavior enabled.")