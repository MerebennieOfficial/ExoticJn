...
-- Merged Circular movement script using LinearVelocity (replaces RunService tweening)
...
-- REPLACED: smoothArcToTarget now uses LinearVelocity to move the HRP along an arc.
local function smoothArcToTarget(targetModel)
    ...
    -- Create Attachment + LinearVelocity on HRP
    local attachment = Instance.new("Attachment")
    attachment.Name = "LV_Attachment"
    attachment.Parent = HRP

    local lv = Instance.new("LinearVelocity")
    lv.Name = "ArcLinearVelocity"
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if finished then
            ...
            pcall(function() if humanoid and oldAutoRotate ~= nil then humanoid.AutoRotate = oldAutoRotate end end)
        ...
        -- estimate next position for velocity using small delta (frame-independent)
        local lookAhead = math.clamp(dt, 0.016, 0.05)
        local tNext = math.clamp(((now - startTime) + lookAhead) / effectiveTime, 0, 1)
        local eNext = easeOutCubic(tNext)
        local radiusNext
        if tNext <= midT then
            local e1n = easeOutCubic(tNext / midT)
            radiusNext = startRadius + (midRadius - startRadius) * e1n
        else
            local e2n = easeOutCubic((tNext - midT) / (1 - midT))
            radiusNext = midRadius + (endRadius - midRadius) * e2n
        end
        radiusNext = math.clamp(radiusNext, MIN_RADIUS, MAX_RADIUS)
        local angleNext = startAngle + deltaAngle * eNext
        local xNext = center.X + radiusNext * math.cos(angleNext)
        local zNext = center.Z + radiusNext * math.sin(angleNext)
        local yNext = myPos.Y + (finalPos.Y - myPos.Y) * eNext
        local posNext = Vector3.new(xNext, yNext, zNext)

        local desiredVelocity = (posNext - posNow) / math.max(lookAhead, 1e-6)

        -- apply to LinearVelocity
        pcall(function()
            if lv and lv.Parent then
                lv.VectorVelocity = desiredVelocity
            end
        end)

        -- orientation: make HRP face toward target position direction smoothly
        ...
        -- optionally align orientation slightly by applying small CFrame yaw set (non-teleporting)
        pcall(function()
            if HRP and HRP.Parent then
                -- only set orientation if magnitude of velocity is small to avoid physics fight; otherwise let physics handle it
                if desiredVelocity.Magnitude < 60 then
                    HRP.CFrame = CFrame.new(HRP.Position, HRP.Position + hrpLook)
                end
            end
        end)

        if t >= 1 then
            -- finish: stop linear velocity and snap to final position+orientation
            if lv and lv.Parent then
            ...
            if HRP and HRP.Parent then
                pcall(function() HRP.CFrame = CFrame.new(finalPos, finalPos + Vector3.new(math.cos(currentDesiredHRPYaw), 0, math.sin(currentDesiredHRPYaw))) end)
            end
            if conn and conn.Connected then conn:Disconnect() end
...
-- UI and the rest of the script left mostly unchanged (buttons/GUI)
...
print("[Merged] Circular LinearVelocity movement + Settings updated and ready.")
