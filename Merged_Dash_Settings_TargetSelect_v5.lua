-- Merged: Circular Dash + Settings V3 (Target-Select + improved sliders UI & behavior)
-- Updated: Removed forced early-stop at a specific degree so spin only ends when it naturally completes.
...
-- Aimlock trigger degrees (unchanged)
...
function smoothCircle480_then_cameraAim(targetModel)
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        -- KEEP Y locked to target's Y during the spin (prevents vertical drift)
        ...
        -- NORMAL end: only stop when the spin has actually finished (t >= 1)
        if t >= 1 then
            if conn then
                conn:Disconnect()
                conn = nil
...
print("[Merged v3] Ready — Settings + Circular Dash merged into one script with Target Select and improved sliders. Forced early-stop removed; spin now only ends when it completes naturally.")