-- DashAssistSettings_v9_merebennie.lua
-- Updated per user request:
--  * M1 Delay textbox placed below "Find nearest" button
--  * Dash alternates to front if you're behind and to back if you're in front
--  * Settings made smaller / compact
--  * Dot-style toggles (dot turns white when ON, blank when OFF)
--  * UI appearance improvements (compact spacing, soft corners)
-- Based on original uploaded file DashAssistSettings_v8_patched.lua
...
-- Safe GUI parenting helper: tries gethui, get_hidden_gui, CoreGui, then PlayerGui
local function safeSetParent(gui)
    ...
    local cg = nil
    local okcg, cgval = pcall(function() return game:GetService("CoreGui") end)
    if okcg then cg = cgval end
    if cg and tryParent(cg) then return cg end

    if LocalPlayer then
        local pg = nil
        local okpg, pgval = pcall(function() return LocalPlayer:FindFirstChild("PlayerGui") end)
        if okpg then pg = pgval end
...
-- New: decide whether to go to front or back based on where you are relative to target
local function computeFinalPosAlternating(targetHRP, myPos)
    ...
    -- if dot > 0 => you're in front of where the target faces, so go to back
    local finalPos
    if toMe:Dot(lookVec) > 0 then
        finalPos = center - lookVec * BEHIND_DISTANCE
    else
        -- you're behind them, go to the front
        finalPos = center + lookVec * BEHIND_DISTANCE
    end
    -- keep Y slightly above ground (match previous behaviour)
...
local function smoothArcToBack(targetModel)
    ...
    -- compute finalPos dynamically to alternate front/back depending on where player currently is
...
local function createSettingsUI()
    ...
    mainFrame.Size = UDim2.new(0,260,0,240) -- smaller compact frame
    ...
    playersLabel.Position = UDim2.new(0,10,0,34)
    ...
    scroll.Size = UDim2.new(1,-20,0,78)
    ...
    findNearestBtn.Size = UDim2.new(0.62,0,0,28)
    findNearestBtn.Position = UDim2.new(0,10,0,140)
    ...
    m1Btn.Name = "M1After"
    m1Btn.Size = UDim2.new(0.34,0,0,28)
    m1Btn.Position = UDim2.new(0.66,6,0,140)
    ...
    -- Delay textbox placed directly BELOW FindNearest (compact)
    local delayLabel = Instance.new("TextLabel")
    delayLabel.Name = "DelayLabel"
    delayLabel.Size = UDim2.new(0.5,0,0,16)
    delayLabel.Position = UDim2.new(0,10,0,176)
    ...
    delayBox.Size = UDim2.new(0,64,0,22)
    delayBox.Position = UDim2.new(0.5,6,0,172)
    ...
    espBtn.Size = UDim2.new(0.34,0,0,28)
    espBtn.Position = UDim2.new(0.66,6,0,172)
    ...
    local function refreshPlayerList()
        ...
        local total = 0
        for _, c in pairs(scroll:GetChildren()) do
            if c:IsA("TextButton") then total = total + c.Size.Y.Offset + 4 end
        end
...
print("[DashAssist v9] Ready. Compact settings UI loaded. Activation requires a player within " .. ACTIVATION_RANGE .. " studs.")
