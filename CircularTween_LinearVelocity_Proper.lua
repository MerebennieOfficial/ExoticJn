...
local function smoothArcToTarget(targetModel)
    ...
    local finalPos = Vector3.new(finalPosLocal.X, myY, finalPosLocal.Z)
    ...
    -- Aimlock (tween to look at target)
    pcall(function()
        local targetPos = targetHRP.Position
        local lookCF = CFrame.new(HRP.Position, targetPos)
        local tween = TweenService:Create(HRP, TweenInfo.new(0.1, Enum.EasingStyle.Linear), { CFrame = lookCF })
        tween:Play()
    end)
    ...
    -- Create Attachment + LinearVelocity constraint (constraint lives under the attachment)
    local att = Instance.new("Attachment")
    att.Name = "DashAttachment"
    att.Parent = HRP
    -- Prefer setting attachment world position near center of mass to reduce unwanted rotation
    pcall(function()
        if HRP.AssemblyCenterOfMass then
            att.WorldPosition = HRP.AssemblyCenterOfMass
        else
    ...
    -- Use world-relative velocity so we can set exact world velocity vectors
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Update LinearVelocity constraint to new velocity
        pcall(function()
        ...
        if t >= 1 then
            ...
            pcall(function() if lv and lv.Parent then lv:Destroy() end end)
            pcall(function() if att and att.Parent then att:Destroy() end end)
            -- snap to final pos & orient towards target
