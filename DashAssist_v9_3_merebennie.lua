...
--// Merebennie DashAssist v9.3 (Delta Mobile Compatible)
--// Changes in v9.3:
--  * UI layout tweaks: Find nearest shorter, M1 nudged right, Delay textbox aligned beside label, ESP aligned with M1
--  * Aimlock now locks only left/right (yaw). It no longer affects up/down (pitch).
--  * ESP highlights selected list target or nearest target reliably (players & NPCs).
--  * Maintains Delta mobile compatibility (touch handlers, safe GUI parenting).
...
-- Aimlock utility: while enabled, force the camera to look at target position (yaw-only)
...
local function startAimlockFor(targetCFrame)
    ...
    aimlockConnection = RunService.Heartbeat:Connect(function()
        ...
        if cam and cam.CFrame and aimlockTargetPos then
            ...
            -- yaw-only: project the target onto the camera's Y (keep camera pitch unchanged)
            local lookTarget = Vector3.new(aimlockTargetPos.X, camPos.Y, aimlockTargetPos.Z)
            pcall(function() cam.CFrame = CFrame.new(camPos, lookTarget) end)
        end
    end)
end
...
local function smoothArcToBack(targetModel)
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        if cam and cam.CFrame then
            -- Keep camera pitch unchanged: compute look target at camera Y level
            local camPos = cam.CFrame.Position
            local lookTarget = Vector3.new(center.X, camPos.Y, center.Z)
            pcall(function() cam.CFrame = CFrame.new(camPos, lookTarget) end)
        end
        if t >= 1 then
            conn:Disconnect()
...
-- Compact Settings UI (layout adjusted)
local function createSettingsUI()
    ...
    -- Find nearest (shorter) with dot indicator
    local findNearestBtn = Instance.new("Frame")
    findNearestBtn.Name = "FindNearest"
    findNearestBtn.Size = UDim2.new(0.48,0,0,30) -- made shorter
    ...
    -- M1 toggle compact with dot (moved right a bit)
    local m1Btn = Instance.new("Frame")
    m1Btn.Name = "M1After"
    m1Btn.Size = UDim2.new(0.3,0,0,30)
    m1Btn.Position = UDim2.new(0.52,8,0,168) -- nudged right from before
    ...
    delayBox.Position = UDim2.new(0.34,8,0,204) -- moved beside label to avoid collision
    ...
    -- ESP toggle aligned with M1 (same column)
    local espBtn = Instance.new("Frame")
    espBtn.Name = "EspToggle"
    espBtn.Size = UDim2.new(0.3,0,0,30)
    espBtn.Position = UDim2.new(0.74,8,0,204) -- aligned right column, matching M1 spacing
...
print("[DashAssist v9.3] Ready. Compact settings UI loaded. Activation requires a target within " .. ACTIVATION_RANGE .. " studs.")
