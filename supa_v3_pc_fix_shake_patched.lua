...
-- << SEQUENCES (added by ChatGPT) >>
-- This table describes the runtime "sequence" used by the script:
-- for each named sequence, the array lists ordered steps with a short description
-- and the delay (in seconds) before the *next* step happens (0 means immediate).
-- You can call printSequence("StickDash") or printSequence("Trigger") in-game
-- (or have the script call it) to show the sequence in the output.
local SEQUENCES = {
    StickDash = {
        { step = "startImmobilize", delay = 0, desc = "Stop player movement, zero velocities, remove forces." },
        { step = "fireQ", delay = 0, desc = "Send simulated 'Q' KeyPress to server and remove BodyVelocity." },
        { step = "pre_lay_wait", delay = 0.2, desc = "Wait 0.2s before laying down (gives server time to process Q)." },
        { step = "changeState_Physics", delay = 0, desc = "Set Humanoid state for physics and tilt character." },
        { step = "anchor_HRP", delay = 0, desc = "Temporarily anchor HumanoidRootPart to prevent PC shaking." },
        { step = "layDuration_move", delay = 0.2, desc = "Maintain laid pose for layDuration (0.2s) while moving near target." },
        { step = "unanchor", delay = 0, desc = "Unanchor HRP to restore normal physics." },
        { step = "changeState_GettingUp", delay = 0, desc = "Change Humanoid state to GettingUp." },
        { step = "stopImmobilize", delay = 0, desc = "Restore walk speed, jump power, and previous settings." },
    },
    Trigger = {
        { step = "check_enabled_and_cooldown", delay = 0, desc = "Abort if disabled or UI cooldown active. Enforce TRIGGER_COOLDOWN (0.3s)." },
        { step = "set_lastTrigger_and_uiOnCooldown", delay = 0, desc = "Mark lastTrigger and start UI cooldown flag." },
        { step = "spawn_stickDash", delay = 0, desc = "Spawn stickDash() in a separate task (runs concurrently)." },
        { step = "start_UI_countdown", delay = 4, desc = "Show cooldown UI countdown for COOLDOWN_DURATION (4 seconds)." },
    }
}

-- Helper to print a sequence to the output for debugging
local function printSequence(name)
    local seq = SEQUENCES[name]
    if not seq then
        print("[SEQUENCE] No sequence named: " .. tostring(name))
        return
    end
    print("[SEQUENCE] "" .. tostring(name) .. "" steps:")
    for i, step in ipairs(seq) do
        local d = step.delay or 0
        print(string.format("  %d) %s  (delay to next: %s) - %s", i, tostring(step.step), tostring(d), tostring(step.desc or "")))
    end
end

-- End of SEQUENCES block
