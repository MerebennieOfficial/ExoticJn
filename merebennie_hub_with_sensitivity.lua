...
-- =========================
-- Touch Sensitivity (Rayfield Slider)
-- Replaces the custom UI slider with Rayfield sliders and runs the touch-sensitivity hook.
-- Creates a slider named "Sensitivity" on both Optimization and Main tabs (if available).
-- =========================
...
do
    ...
    local function setupCameraHook()
        ...
        -- As a safe fallback (non-invasive), keep a RenderStepped stub to mark hookActive
        if not success then
            RunService.RenderStepped:Connect(function()
                if UserInputService.TouchEnabled then
                    -- no-op fallback
    ...
    local function applySensitivity(value)
        ...
        saveSettings(value)
        -- If cameraInputModule is hooked, it's already using currentSensitivity
        -- Provide optional console/log feedback for executors that can read prints
        pcall(function()
            print(string.format("[TouchSensitivity] Applied sensitivity = %.2f", currentSensitivity))
        end)
    end

    -- create a single callback which Rayfield sliders will call
    local function sliderCallback(val)
        -- Rayfield slider can pass values with many decimals; clamp and round to 2 decimals for saving
        local clamped = math.clamp(val, CONFIG.MIN_SENSITIVITY, CONFIG.MAX_SENSITIVITY)
        local rounded = math.floor(clamped * 100 + 0.5) / 100
        applySensitivity(rounded)
        -- Optional tiny notification using Rayfield if available
        pcall(function()
            Rayfield:Notify({
                Title = "Touch Sensitivity",
                Content = "Set to " .. tostring(rounded),
                Duration = 1.5
            })
        end)
    end

    -- Ensure Rayfield Window/Tab variables exist (they should from the main hub script)
    -- Add the slider to Optimization (Tab) and Main (MainTab) if present
    local sliderParams = {
        Name = "Sensitivity",
    ...
    pcall(function()
        if Tab and Tab.CreateSlider then
            Tab:CreateSlider(sliderParams)
        end
    end)

    pcall(function()
        if MainTab and MainTab.CreateSlider then
            MainTab:CreateSlider(sliderParams)
        end
    end)

    -- Also expose a convenient global for manual changes from console
    getgenv().MerebennieTouchSensitivity = getgenv().MerebennieTouchSensitivity or {}
    getgenv().MerebennieTouchSensitivity.get = function() return currentSensitivity end
    getgenv().MerebennieTouchSensitivity.set = function(v) applySensitivity(v) end

    -- Apply current sensitivity immediately
    applySensitivity(currentSensitivity)

    -- Notify if camera hook failed
    if not cameraHooked then
        pcall(function()
            print("[TouchSensitivity] Warning: auto-hook did not succeed. The fallback may still scale touch input on some clients.")
            Rayfield:Notify({
                Title = "Touch Sensitivity",
                Content = "Hook not fully succeeded. Try reloading character or executor.",
                Duration = 3
            })
        end)
    end
