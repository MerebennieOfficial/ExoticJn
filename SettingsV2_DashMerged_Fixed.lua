-- Settings V2 + Dash Settings (fixed)
-- Standalone LocalScript
-- Fixes:
--  1) Slider values persist across opening/closing the Adjust (Dash) panel (even if not saved).
--  2) Save button uses persistent values (doesn't depend on panel being open).
--  3) Save button styled white and doesn't gray on click (AutoButtonColor=false).
--  4) Interactive buttons do not show the default gray click state.
--  5) Adjust panel now appears centered on screen.
...
-- persistent in-memory slider values (survive panel open/close until player leaves)
...
do
    ...
    if type(attr) == "string" then
        pcall(function()
            ...
            -- if saved has slider entries, copy into currentSliderValues so panel picks them up
...
saveBtn.BackgroundColor3 = Color3.fromRGB(245,245,245) -- white-like
saveBtn.Text = "Save"
saveBtn.TextColor3 = Color3.fromRGB(0,0,0)
saveBtn.Font = Enum.Font.GothamBold
saveBtn.TextSize = 14
saveBtn.AutoButtonColor = false -- prevent default gray click flash
Instance.new("UICorner", saveBtn).CornerRadius = UDim.new(0,8)
saveBtn.Parent = buttonHolder

saveBtn.MouseButton1Click:Connect(function()
    pcall(function() clickSound:Play() end)
    -- collect slider values from persistent table (works even if panel closed)
    local sliderValues = {}
    local titleLabels = {"Dash speed", "Dash Degrees", "Dash gap"}
    for _, name in ipairs(titleLabels) do
        local v = currentSliderValues[name]
        if v == nil and Sliders[name] and type(Sliders[name]) == "function" then
            v = Sliders[name]()
        end
        sliderValues[name] = v or 0
    end
...
local function createDashPanel()
    ...
    for i, name in ipairs(TitleLabels) do
        ...
        -- Initialization order: prefer in-memory currentSliderValues (persist across open/close),
        -- otherwise fallback to savedSettings stored on player attribute, otherwise default 0.
        local initValue = 0
        if currentSliderValues[name] ~= nil then
            ...
            if savedSettings and savedSettings.Sliders and savedSettings.Sliders[name] then
                savedValue = tonumber(savedSettings.Sliders[name])
            end
            if savedValue and type(savedValue) == "number" then
                initValue = math.clamp(math.floor(savedValue), 0, 100)
            end
        end
        value = initValue
        ...
        local function update(absX)
            ...
            -- persist to in-memory table immediately so closing won't lose it
        ...
        -- Expose value getter into top-level Sliders table (for backwards compatibility)
...
print("Standalone Settings GUI V2 (with Dash Settings + Save) loaded. (Fixed version)")