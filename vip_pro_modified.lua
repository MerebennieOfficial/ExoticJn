-- Modified "vip pro" script
-- Changes made:
-- 1) Replaced ImageButton with a simple movable TextButton (draggable).
-- 2) Added safety checks so the button works for NPCs (dummy) as well as players.
-- 3) Kept original functionality and structure; adjusted only where necessary for safety and the UI change.
...
-- Helper: safe HRP getter for target (works for dummy NPCs and player characters)
...
if ui.TouchEnabled and not (ui.KeyboardEnabled and ui.GamepadEnabled and gui:IsTenFootInterface()) then
	...
	-- Create a simple movable TextButton instead of an ImageButton
	TextButton.Parent = ScreenGui
	TextButton.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
	TextButton.BackgroundTransparency = 0.300
	TextButton.BorderColor3 = Color3.fromRGB(0, 0, 0)
	TextButton.BorderSizePixel = 0
	TextButton.Position = UDim2.new(0.15976575, 0, 0.0619156398, 0)
	TextButton.Size = UDim2.new(0.0889796019, 0, 0.16869919, 0)
	...
	-- Make the button draggable (works for touch and mouse)
...
function GetTarget()
	...
	if ui.KeyboardEnabled then
		dummy = game.Workspace.Live:FindFirstChild("Weakest Dummy")
		local dummyP = dummy and dummy.HumanoidRootPart and dummy.HumanoidRootPart.CFrame.Position or Vector3.new(9e9,9e9,9e9)
		...
		if dummy and dummyP and (dummyP - mousepos).Magnitude < lastmagnitude then
			target = dummy
			addHighlight(target)
		else
			if target and target.Character then addHighlight(target.Character) end
		end
	end
end
	...
	if input.UserInputType == Enum.UserInputType.Keyboard then
			...
			if checkPlayer or game.Workspace.Live:FindFirstChild("Weakest Dummy") then
				...
				if toogle then
					...
					if target == dummy then
						...
						if target and target.Character then removeHighlight(target.Character) end
						target = nil
					end
				end
			end
		...
		if toogle and target then
			if input.KeyCode == Enum.KeyCode.C then
				...
				-- Safe target Y check (works for both NPCs and players)
				local hrp = getTargetHRP(target)
				if p.Position.Y <= 442 and hrp and hrp.Position.Y <= 448 then
					if target ~= nil and target == dummy then
						...
						if (target.HumanoidRootPart.CFrame.Position - p.CFrame.Position).Magnitude <= distance and f == true then
							-- main behavior preserved from original; kept unchanged
							f = false
							noClip = false
							m = true
							a = true
...
-- Replace ImageButton click usage with TextButton (safe check included)
if TextButton then
	TextButton.MouseButton1Click:Connect(function()
		local p = game.Players.LocalPlayer.Character.HumanoidRootPart
		local hrp = getTargetHRP(target)
		if p and hrp and p.Position.Y <= 442 and hrp.Position.Y <= 448 then
			if toogle and target then
				if target ~= nil and target == dummy then
					p = game.Players.LocalPlayer.Character.HumanoidRootPart
					if (target.HumanoidRootPart.CFrame.Position - p.CFrame.Position).Magnitude <= distance and f == true then
