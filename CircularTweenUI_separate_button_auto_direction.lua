...
-- CircularTweenUI with ESP + Separate Dash Button + Automatic Front/Back selection
-- Dash Assist: yaw-only aimlock, animations, sounds, mobile+PC+controller support
-- Activation range: 35, Arc approach radius: 15
...
local function getCharacterFromPlayer(pl)
    ...
    if typeof(pl) == "Instance" and pl:IsA("Model") then return pl end
...
-- Determine automatic direction: if you are in front of the target (dot>0), dash to Back; if behind (dot<0), dash to Front
local function chooseDirectionAuto(targetHRP)
    local myPos = HRP.Position
    local targetPos = targetHRP.Position
    local rel = (myPos - targetPos)
...
-- Unified smooth arc function; mode can be "Back" or "Front". If mode is nil, it will be chosen automatically.
...
-- UI: settings (no dash button here)
local function createSettingsUI()
    ...
    miniButton.Position = UDim2.new(0.02,0,0.6,0)
    miniButton.Text = ">"
            ...
            if pl ~= LocalPlayer then
                ...
                btn.MouseButton1Click:Connect(function()
                    ...
                    for _, c in pairs(scroll:GetChildren()) do
                        if c:IsA("TextButton") then c.BackgroundColor3 = Color3.fromRGB(34,34,34) end
                    end
    ...
    findNearestBtn.MouseButton1Click:Connect(function()
        ...
        if findNearestEnabled then
            for _, c in pairs(scroll:GetChildren()) do if c:IsA("TextButton") then c.BackgroundColor3 = Color3.fromRGB(34,34,34) end end
            selectedPlayer = nil
        end
    end)
    findNearestDot.Visible = findNearestEnabled
    ...
    m1Box.FocusLost:Connect(function(enter)
    ...
    -- expose function to update ESP on selection change
    return {
        UpdateESP = function()
            local target = nil
            ...
            if target then applyESPToModel(target) else clearESP() end
        end
    }
end
...
local function createDashButton()
    ...
    -- Activation on release (tap/click)
    btn.MouseButton1Click:Connect(function()
        if busy then return end
        -- choose character based on settings
        ...
        if chosenCharacter then
            smoothArcTo(chosenCharacter, nil) -- nil mode => automatic choice
        end
    end)
...
-- INIT UIs
local settings = createSettingsUI()
createDashButton()
...
-- KEYBINDS support (same activation logic as dash button)
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if activated then
        ...
        if chosenCharacter then smoothArcTo(chosenCharacter, nil) end
    end
end)

print("[CircularTweenUI] Ready. Dash button is separate from settings. Front/Back direction is chosen automatically based on your position relative to the target.")
