...
-- Updated: increased settings width; added Land Studs and Dash speed adjustable UI (exactly based on uploaded file)
-- NOTE: Keep everything exactly like the original file's behavior, only implemented requested UI changes.
...
-- Land studs (user-adjustable; default 5)
local LAND_STUDS = 5
local BEHIND_DISTANCE = LAND_STUDS
local FRONT_DISTANCE = LAND_STUDS
-- Dash duration (seconds) - adjustable via UI (Dash speed)
...
{Size = UDim2.new(0, 360, 0, 270)}):Play()
...
-- --- New adjustable panels: Land Studs + Dash speed -------------------
local adjustFrame = Instance.new("Frame")
adjustFrame.Size = UDim2.new(1, -20, 0, 40)
adjustFrame.Position = UDim2.new(0, 10, 0, 220)
...
-- Land Studs control (integer)
local landControl = createAdjustControls(leftPanel, "Land Studs", LAND_STUDS, 1, 20, 1, function(v) return tostring(math.floor(v)) end,
    function(v)
        LAND_STUDS = math.floor(v)
        BEHIND_DISTANCE = LAND_STUDS
        FRONT_DISTANCE = LAND_STUDS
    end)

-- Dash speed control (seconds) - labeled "Dash speed"
...
print("[Merged] Circular tween + Settings updated and ready. LAND_STUDS="..tostring(LAND_STUDS).." TOTAL_TIME="..tostring(TOTAL_TIME))
