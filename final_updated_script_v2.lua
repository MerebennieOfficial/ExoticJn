...
local function smoothArcToTarget(targetModel)
    ...
    local targetHead = targetModel:FindFirstChild("Head") or targetHRP
    local center = (targetHead and targetHead.Position) or (targetHRP and targetHRP.Position)
    local myPos = HRP.Position
    ...
    conn = RunService.Heartbeat:Connect(function(delta)
        ...
        local targetPosForFacing = (targetHead and targetHead.Position) or (targetHRP and targetHRP.Position)
        local toTargetFromHRP = targetPosForFacing - posNow
        ...
        -- Instant aimlock (X-axis only, faster)
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            pcall(function()
                local camPos = cam.CFrame.Position
                local aimTargetPos = (targetHead and targetHead.Position) or (targetHRP and targetHRP.Position)
                local lookDir = (aimTargetPos - camPos)
                -- Zero out Y difference to restrict to left/right only
        ...
        if t >= 1 then
            ...
            pcall(function()
                if HRP and HRP.Parent then
                    ...
                    -- Face exactly the same horizontal direction the target is facing on landing
                    local targetLookVec = nil
                    if targetHead and targetHead.Parent and targetHead:IsA("BasePart") then
                        targetLookVec = (targetHead.CFrame.LookVector)
                    elseif targetHRP and targetHRP.Parent then
                        targetLookVec = (targetHRP.CFrame.LookVector)
                    else
                        targetLookVec = Vector3.new(math.cos(currentDesiredHRPYaw), 0, math.sin(currentDesiredHRPYaw))
                    end
                    if targetLookVec then
                        targetLookVec = Vector3.new(targetLookVec.X, 0, targetLookVec.Z)
                        if targetLookVec.Magnitude <= 0 then targetLookVec = Vector3.new(1,0,0) end
                        HRP.CFrame = CFrame.new(posNow, posNow + targetLookVec)
                    end
                end
            end)
