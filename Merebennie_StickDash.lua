...
local COOLDOWN_DURATION = 7 -- reduced from 7.2 to 7 seconds
...
local function stickDash()
    ...
    -- original 0.18 laydown delay (unchanged)
...
-- === Main toggle button (45x45, smooth edges) — BLUE (modified size, font, animation & sound) ===
local Button = Instance.new("TextButton")
Button.Name = "StickDashToggle"
Button.Size = UDim2.new(0, 45, 0, 45) -- changed to 45x45
Button.Position = UDim2.new(0.4, 0, 0.8, 0)
Button.Text = "On" -- changed to "On" (default because enabled = true)
Button.Font = Enum.Font.GothamBold -- fallback; we'll attempt to set Fredoka via Font API below
Button.TextSize = 18 -- slightly larger, will scale visually
Button.TextScaled = true -- keep it proportional inside the 45x45
Button.BackgroundColor3 = Color3.fromRGB(66,150,255) -- tiny bit lighter blue when ON
...
-- Try to apply Fredoka font properly (roblox newer font system). Fallback to default if unavailable.
pcall(function()
    if Font and Font.new then
        -- use the FredokaOne family if available in the runtime
        Button.FontFace = Font.new("rbxasset://fonts/families/FredokaOne.json")
    end
end)

-- Add sound (Cartoon bubble button Sound) and small press animation
local btnSound = Instance.new("Sound")
btnSound.Name = "ToggleSound"
btnSound.SoundId = "rbxassetid://5852470908"
btnSound.Parent = Button
btnSound.Volume = 1

-- store original size for animation resets
local origButtonSize = Button.Size
local pressTween = TweenService:Create(Button, TweenInfo.new(0.06, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Size = UDim2.new(origButtonSize.X.Scale, math.floor(origButtonSize.X.Offset * 0.88), origButtonSize.Y.Scale, math.floor(origButtonSize.Y.Offset * 0.88))})
local releaseTween = TweenService:Create(Button, TweenInfo.new(0.12, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out), {Size = origButtonSize})
...
do
    ...
    Button.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            -- start press animation & sound (touch/mouse)
            pcall(function()
                pressTween:Play()
                if btnSound and btnSound.Play then
                    btnSound:Play()
                end
            end)
            ...
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
                    -- release animation
                    pcall(function() releaseTween:Play() end)
                end
            end)
        end
    end)
...
-- Toggle On/Off (no logic/timing changes; just gating) + ensure sound & release tween on click
Button.MouseButton1Click:Connect(function()
    enabled = not enabled
    if enabled then
        Button.Text = "On"
        Button.BackgroundColor3 = Color3.fromRGB(66,150,255) -- lighter blue ON
    else
        Button.Text = "Off"
        Button.BackgroundColor3 = Color3.fromRGB(130,130,130) -- lighter OFF gray
    end
    -- make sure release animation plays (in case click without ending inputChanged)
    pcall(function() releaseTween:Play() end)
    pcall(function()
        if btnSound and btnSound.Play then
            btnSound:Play()
...
cdCorner.CornerRadius = UDim.new(0, 14)
...
local pulseTween = TweenService:Create(CooldownFrame, pulseInfo, {BackgroundTransparency = 0.18})
...
local function startUICooldown(duration)
    ...
    while remaining > 0 do
        ...
        CooldownLabel.Text = string.format("Kiba V3 : %.1fs", remaining)
        task.wait(0.1)
    end
