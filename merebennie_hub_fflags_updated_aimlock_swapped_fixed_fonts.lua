...
-- NOTE: Swapped creation order so Main appears on the RIGHT and Optimization on the LEFT.

-- =========================
-- Font Changer (Optimization tab) - Adds 13 font options applied to all Text UI (PlayerGui & CoreGui)
-- =========================
do
    local Fonts = {
        "SourceSans", "SourceSansBold", "SourceSansItalic", "SourceSansLight", "Roboto",
        "Gotham", "GothamBold", "RobotoCondensed", "Arcade", "Cartoon", "Mechanic", "Highway", "Legacy"
    }

    local FontMap = {
        SourceSans = Enum.Font.SourceSans,
        SourceSansBold = Enum.Font.SourceSansBold,
        SourceSansItalic = (Enum.Font.SourceSansItalic and Enum.Font.SourceSansItalic) or Enum.Font.SourceSans,
        SourceSansLight = (Enum.Font.SourceSansLight and Enum.Font.SourceSansLight) or Enum.Font.SourceSans,
        Roboto = Enum.Font.Gotham or Enum.Font.SourceSans,
        Gotham = Enum.Font.Gotham or Enum.Font.SourceSans,
        GothamBold = Enum.Font.GothamBold or Enum.Font.SourceSansBold,
        RobotoCondensed = Enum.Font.Roboto or Enum.Font.SourceSans,
        Arcade = (Enum.Font.Arcade and Enum.Font.Arcade) or Enum.Font.SourceSans,
        Cartoon = (Enum.Font.Cartoon and Enum.Font.Cartoon) or Enum.Font.SourceSans,
        Mechanic = (Enum.Font.Mechanic and Enum.Font.Mechanic) or Enum.Font.SourceSans,
        Highway = (Enum.Font.Highway and Enum.Font.Highway) or Enum.Font.SourceSans,
        Legacy = Enum.Font.Legacy or Enum.Font.SourceSans
    }

    local function applyFontToGui(fontEnum)
        local Players = game:GetService("Players")
        local player = Players.LocalPlayer
        pcall(function()
            for _, gui in ipairs({player:FindFirstChild("PlayerGui"), game:GetService("CoreGui")}) do
                if gui then
                    for _, obj in ipairs(gui:GetDescendants()) do
                        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") or obj:IsA("TextLabel") then
    ...
    -- Try to attach to Optimization tab; try common variable names. If not found, create a fallback button in MainTab.
    local created = false
    local function tryCreateDropdown(tab)
        ...
        if type(tab.CreateDropdown) == "function" then
            tab:CreateDropdown({
                Name = "Change Font",
                Info = "Apply to all Text UI (PlayerGui & CoreGui)",
                Options = Fonts,
                CurrentOption = Fonts[1],
                Flag = "FontChanger",
                Callback = function(choice)
                    local fontEnum = FontMap[choice] or Enum.Font.SourceSans
                    applyFontToGui(fontEnum)
                    if getgenv then getgenv().SelectedFont = choice end
                    if Rayfield and type(Rayfield.Notify) == "function" then
                        pcall(function() Rayfield:Notify({Title = "Font Changed", Content = choice, Duration = 2}) end)
                    end
                end,
            })
            return true
    ...
    pcall(function()
        created = created or tryCreateDropdown(OptimizationTab)
        created = created or tryCreateDropdown(getgenv and getgenv().Optimization or nil)
        created = created or tryCreateDropdown(Tab)
        created = created or tryCreateDropdown(getgenv and getgenv().Tab or nil)
        created = created or tryCreateDropdown(MainTab)
        -- Fallback: create a button in MainTab to cycle fonts
        if not created and MainTab and type(MainTab.CreateButton) == "function" then
            MainTab:CreateButton({
                Name = "Cycle Fonts (fallback)",
                Callback = function()
                    local FontsLocal = Fonts
                    local current = getgenv and getgenv().SelectedFont or FontsLocal[1]
                    local idx = 1
                    for i, v in ipairs(FontsLocal) do if v == current then idx = i break end end
                    local nextIdx = (idx % #FontsLocal) + 1
                    local choice = FontsLocal[nextIdx]
                    local fontEnum = FontMap[choice] or Enum.Font.SourceSans
                    applyFontToGui(fontEnum)
                    if getgenv then getgenv().SelectedFont = choice end
                    if Rayfield and type(Rayfield.Notify) == "function" then
...
-- Create Main tab first (will appear on the right in Rayfield by creation order in many setups)
...
-- =========================
-- Professional Resolution toggle moved to MAIN tab (per request)
-- =========================
...
-- =========================
-- NEW: Aimlock button (runs the Aimlock UI/function provided by the user)
-- =========================
  ...
  if target and LocalPlayer and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
   ...
   if useHead then
    ...
    if nearestPart and nearestPart:IsA("BasePart") then
     ...
     desiredAim = (targetHRP and targetHRP:IsA("BasePart")) and targetHRP.Position or (targetHead and targetHead:IsA("BasePart") and targetHead.Position) or nil
    end
   end

   if desiredAim then
    -- smooth aim position
    if not aimPosSmoothed then
     aimPosSmoothed = desiredAim
    else
     aimPosSmoothed = aimPosSmoothed + (desiredAim - aimPosSmoothed) * aimPosLerpSpeed
    end

    -- APPLY RIGHT OFFSET: compute camera-right vector and add small offset
    local camRight = Camera.CFrame.RightVector
    local displayedAim = aimPosSmoothed + (camRight * rightOffset)

    -- set camera to look at the displayed (offset) aim position
    local camPos = Camera.CFrame.Position
    local camCF = CFrame.new(camPos) * CFrame.new(0, heightOffset, 0)
    local lookCF = CFrame.lookAt(camCF.Position, displayedAim) * CFrame.Angles(tiltDown, 0, 0)
    Camera.CFrame = Camera.CFrame:Lerp(lookCF, currentLerp)

    -- CHARACTER YAW ROTATION: rotate toward displayedAim while aimlock is active, but skip if ragdolled
    local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    local humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
    if root and displayedAim and humanoid and not isRagdolled(humanoid) then
     -- compute horizontal (XZ) direction to displayed aim point
     local lookVec = displayedAim - root.Position
     lookVec = Vector3.new(lookVec.X, 0, lookVec.Z)
     if lookVec.Magnitude > 0.001 then
      local targetCF = CFrame.lookAt(root.Position, root.Position + lookVec)
      -- Lerp only rotation (position is preserved)
      root.CFrame = root.CFrame:Lerp(targetCF, rotationLerp)
     end
    end
   end
  end
...
-- Button press behavior: play sound + press animation + toggle aimlock
Button.MouseButton1Click:Connect(function()
 -- play local click sound
 SoundService:PlayLocalSound(clickSound)

 -- press animation: shrink a bit, then release tween back
 local pressTween = TweenService:Create(Button, pressTweenInfo, {Size = pressedSize})
 local releaseTween = TweenService:Create(Button, releaseTweenInfo, {Size = normalSize})
 pressTween:Play()
 pressTween.Completed:Connect(function()
  releaseTween:Play()
 end)

 -- toggle aimlock state
 aimlockEnabled = not aimlockEnabled
 if aimlockEnabled then
  target = acquireNearestTarget()
  -- initialize aimPosSmoothed to the nearest part for smoothness
  if target and LocalPlayer and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
   local startPart = getNearestPartOfModel(target, LocalPlayer.Character.HumanoidRootPart.Position)
   if startPart then aimPosSmoothed = startPart.Position end
  end
...
-- make sure helper functions are available in this file scope
-- (acquireNearestTarget and getNearestPartOfModel are defined above and referenced)

-- make sure the UI persists on respawn
ScreenGui.ResetOnSpawn = false
        end)
    end,
})
