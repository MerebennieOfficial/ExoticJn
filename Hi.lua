...
local function smoothArcToTarget(targetModel)
    ...
    -- Setup LinearVelocity and Attachment on HRP (reuse if present)
    ...
    local lv = HRP:FindFirstChild("DashLinearVelocity")
    if not lv then
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        -- Ground-aware vertical calculation to prevent floating when arcing
        local startGroundY = getGroundY(myPos.X, myPos.Z) or myPos.Y
        local finalGroundY = getGroundY(finalPos.X, finalPos.Z) or finalPos.Y
        -- create an arc lift that peaks at the middle (0.5)
        local verticalLift = 1.2 * math.sin(math.pi * e)
        local y = (startGroundY + (finalGroundY - startGroundY) * e) + verticalLift

        local posNow = Vector3.new(x, y, z)

        -- aimlock / HRP yaw smoothing — faster via AIMLOCK_MULTIPLIER
        ...
        local yawFactor = math.clamp(e * AIMLOCK_MULTIPLIER, 0, 1)
        local hrpYawNow = startHRPYaw + deltaHRPYaw * yawFactor
        local hrpLook = Vector3.new(math.cos(hrpYawNow), 0, math.sin(hrpYawNow))

        -- Use LinearVelocity to move HRP smoothly towards posNow
        local dt = math.max(1/240, now - lastTick)
        lastTick = now
        local currentPos = HRP.Position
        local neededVel = (posNow - currentPos) / dt

        pcall(function()
            if lv and lv.Parent then
                lv.VectorVelocity = neededVel
            end
            -- Orient HRP to face hrpLook (only yaw) without changing vertical from LV
            pcall(function()
                local curPos = HRP.Position
                HRP.CFrame = CFrame.new(curPos, curPos + hrpLook)
            end)
        end)
        ...
        if t >= 1 then
            if conn and conn.Connected then conn:Disconnect() end
            -- stop velocity and snap to final
            pcall(function() if lv and lv.Parent then lv.VectorVelocity = Vector3.new(0,0,0) end end)
            ...
            -- cleanup
            pcall(function() if lv and lv.Parent then lv:Destroy() end end)
            delay(0.03, function() pcall(function() if attachment and attachment.Parent then attachment:Destroy() end end) end)
            busy = false
        end
    end)
