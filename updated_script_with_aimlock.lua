...
local function smoothArcToTarget(targetModel)
    ...
    conn = RunService.Heartbeat:Connect(function(delta)
        ...
        -- Aimlock yaw-only (left/right), back or front torso selection
        if cam and cam.CFrame and targetHRP and targetHRP.Parent then
            pcall(function()
                local camPos = cam.CFrame.Position
                -- Determine back vs front lock position
                local offsetPos
                if forwardDot > 0 then
                    -- behind target
                    offsetPos = targetHRP.Position - lookVec * BEHIND_DISTANCE
                else
                    -- front target
                    offsetPos = targetHRP.Position + lookVec * FRONT_DISTANCE
                end
                -- calculate yaw only
                local dir = offsetPos - camPos
                dir = Vector3.new(dir.X, 0, dir.Z).Unit
                local yawLook = Vector3.new(dir.X, 0, dir.Z)
                cam.CFrame = CFrame.new(camPos, camPos + yawLook)
            end)
        end
