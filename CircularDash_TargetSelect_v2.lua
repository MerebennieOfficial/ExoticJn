...
-- Choose target respecting selectedPlayer and MAX_RANGE; fall back to nearest
...
function chooseTarget()
    -- If a specific player is selected, prefer them.
        ...
        if selectedPlayer.Character and selectedPlayer.Character.Parent then
            ...
            if hrp and hum and hum.Health > 0 and HRP then
                ...
                if d <= MAX_RANGE then
                    ...
                    -- Selected player exists but is out of range.
                    -- Keep the selection (do NOT clear selectedPlayer), but do not return a target.
                    return nil
                end
            else
    ...
    -- No player explicitly selected (or selection was cleared because player invalid/dead):
    -- fall back to nearest valid target (players + NPCs) within MAX_RANGE.
